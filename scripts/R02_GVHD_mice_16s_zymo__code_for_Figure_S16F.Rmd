...
gvhdall <- read_csv('../data/204_mice_diet_gvhd_all.csv') %>% 
  dplyr::rename(day_relative_to_transplant = day)

meta <- gvhdall %>%
  dplyr::select(-Taxon, -relab) %>% 
  distinct() 
```
...
transplant_CFU <- gvhd_counts %>% 
    dplyr::select( day_relative_to_transplant, cf_us_per_gram_stool,abx_treatment:gvhd_grp) %>% 
...
save_ggplot_data_to_excel(transplant_CFU_plot, transplant_CFU, "../mouse_data.xlsx", "Sup_Figure_9h_bmt_cfu", col_names = c(
  "xvar" = "Treatment + Day",
  "cf_us_per_gram_stool" = "CFUs per gram stool",
  "grp" = "Treatment Group"))
