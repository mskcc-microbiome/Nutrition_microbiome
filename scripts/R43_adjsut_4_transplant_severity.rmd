...
found <- read_csv('../data/R43_diet_pt_CI_needed.csv')
...
cleaned_ci_data %>% write_csv('../data/R43_cleaned_ci_data_diet.csv')
```
...
cleaned_ci_data <- read_csv('../data/R43_cleaned_ci_data_diet.csv')

meta_CI <- meta %>%
  left_join(cleaned_ci_data %>% select(pid, ci_cleaned_numeric)) %>% 
...
results_df |> write_csv('../data/R43_results_df_adjusting_for_CI.csv')
```
...
cleaned_effects <- results_df %>%
  ...
  mutate(
    ...
    clean_term = term %>%
      ...
      str_replace("empiricalTRUE:", "abx * ") %>%
      str_replace_all("_", " ") %>% 
      str_replace("ci cleaned numeric", "Comorbidity index")
  ) %>%
...
df_pt_stool_grouped %>% count(disease_lineage, intensity)

df_pt_stool_grouped |> write_csv('../data/R43_cleaned_patients_disease_lineage.csv')
```
...
results_df_disease |> write_csv('../data/R43_results_df_adjusting_for_Cancer_type.csv')
```
