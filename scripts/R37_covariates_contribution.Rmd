...
plot_data <- covariate_data |>
  ...
  mutate(
    category = case_when(
      variable_name %in% c("pid", "Sex") ~ "Inter-Patient Variability",
      variable_name %in% c("ABX exposure (last 2 days)", "Week Relative to Transplant") ~ "Sample-Level Non-Diet Metadata",
      str_starts(variable_name, "fg") ~ "Dietary Factors",
      variable_name == "Enteral Nutrition" ~ "Dietary Factors",
      # TRUE ~ ... acts as a catch-all for anything not matched above.
      TRUE ~ "Clinical Factors"
    ),
    # Let's make the variable names more descriptive for the publication.
    # `str_replace_all` helps clean up the labels.
...
plot_all_covariates <- plot_data |>
  ...
  theme(legend.position = "bottom") # Place legend at the bottom for a wider plot.
...
plot_zoomed_in <- plot_data |>
  ...
  # Adding text labels helps the reader see the precise R² value for each bar.
