...
meta %>% 
  dplyr::count(experiment_no, abx_treatment, diet_treatment, day) %>% 
...
alpha_change  
save_ggplot_data_to_excel(alpha_change, alpha_div_df, "../mouse_data.xlsx", "Sup_Figure_8a_alpha_diversity_mouse_stool", col_names = c(
  "simpson_reciprocal" = "Stool Alpha Diversity (Simpson reciprocal)",
  "grp" = "Treatment Group",
...
# color by abx    
pcoa_df <- beta_all %>% 
  mutate(day = ifelse(day == 0, 1, day)) %>% #corrects earlier misabeling
  mutate(day = factor(day), experiment_no = factor(experiment_no)) %>% 
  arrange(abx_treatment,desc(diet_treatment),  day) %>% 
  mutate(xvar = str_glue('{abx_treatment}__{diet_treatment}__{day}')) %>% 
  mutate(grp = str_glue('{abx_treatment}__{diet_treatment}')) %>% 
...
beta_4  
save_full_dataframe_to_excel(pcoa_df %>% select(c(V1, V2, grp, day, experiment_no)), "../mouse_data.xlsx", "Sup_Figure_8b_mouse_PCOA", col_names = c(
  "V1" = "PCOA Axis 1",
  "V2" = "PCOA Axis 2",
  "experiment_no" = "Experiment Number",
  "day" = "Day",
...
kept_asv <- asv_target %>%
  group_by(Taxon) %>%
  dplyr::count(relab > 0.0001) %>%
  dplyr::rename(criteria = names(.)[2]) %>%
...
pbs_sig_maaslin_plt <- all_res_abx %>%
...
abx_vol_d3 <-  pbs_sig_maaslin_plt %>%
...
abx_vol_d3
save_ggplot_data_to_excel(abx_vol_d3, pbs_sig_maaslin_plt, "../mouse_data.xlsx", "Sup_Figure_8c_day1_vs_day3", col_names = c(
  "coef" = "Maaslin Model Coefficient",
  "neglog10_q" = "- log 10 Q value",
  "sig_" = "Significant at qval <0.05 and coef > 1",
  "seq_id" = "ASV seq id number",
  "qval" = "Q Value"))
```
...
pbs_sig_maaslin_plt_d6 <- all_res_abx %>% 
  filter(metadata == 'treatmentbiapenem_vehicle:day6') %>% 
  mutate(sig_ = if_else(qval < 0.05 & coef > 1, 'pos', if_else(qval < 0.05 & coef < -1, 'neg', 'not'))) %>% 
  mutate(neglog10_q = -log10(qval))
abx_vol_d6 <- pbs_sig_maaslin_plt_d6 %>%
...
save_ggplot_data_to_excel(abx_vol_d6, pbs_sig_maaslin_plt_d6, "../mouse_data.xlsx", "Sup_Figure_8c_day1_vs_day6", col_names = c(
  "coef" = "Maaslin Model Coefficient",
  "neglog10_q" = "- log 10 Q value",
  "sig_" = "Significant at qval <0.05 and coef > 1",
  "seq_id" = "ASV seq id number",
...
save_ggplot_data_to_excel(abx_asvs_relab_plots[[1]], abx_asvs_relab_plots[[1]]$data, "../mouse_data.xlsx", "Sup_Figure_8d_seq_30", col_names = c(
  "day" = "Day",
  "relab" = "Relative Abundance",
  "u_mouse_id" = "Mouse Identifier"))
save_ggplot_data_to_excel(abx_asvs_relab_plots[[2]], abx_asvs_relab_plots[[2]]$data, "../mouse_data.xlsx", "Sup_Figure_8d_seq_38", col_names = c(
  "day" = "Day",
  "relab" = "Relative Abundance",
  "u_mouse_id" = "Mouse Identifier"))
save_ggplot_data_to_excel(abx_asvs_relab_plots[[3]], abx_asvs_relab_plots[[3]]$data, "../mouse_data.xlsx", "Sup_Figure_8d_seq_48", col_names = c(
  "day" = "Day",
  "relab" = "Relative Abundance",
  "u_mouse_id" = "Mouse Identifier"))
