...
meta_table |> write_tsv('../data/R38_batch1_meta.tsv')
```
...
generate_pca_plot <- function(counts_matrix, meta_subset, group_variable, plot_title) {
  ...
  # Returning a list so we can access both the visualization and the numbers behind it
  return(list(
    plot = pca_plot,
    data = pca_plot$data
  ))
...
colonization_plot <- plot_grid(pca_si_col[[1]], pca_li_col[[1]], ncol = 2, labels = c("A", "B"))
colonization_plot
...
save_ggplot_data_to_excel(pca_li_suc[[1]], pca_li_suc[[2]], "../mouse_data.xlsx", "Sup_Figure_9f_pca_e_faec_sugar", col_names = c(
  "PC1" = "PCA Axis 1",
  "PC2" = "PCA Axis 2",
  "group" = "Treatment Group"))
# Combine into a single figure
sucrose_plot <- plot_grid(pca_si_suc[[1]], pca_li_suc[[1]], ncol = 2, labels = c("C", "D"))

save_plot("../data/pca_sucrose_effect.pdf", sucrose_plot, base_height = 6, base_width = 12)
print(sucrose_plot)
...
```{r}
# the p value for the LI sucrose effects pca
pca_data <- pca_li_suc$data

pc1_test <- t.test(PC1 ~ group, data = pca_data)
```
```{r}
# checking if we have enough samples to even bother with normality testing
# (Shapiro-Wilk really needs at least 3 samples per group to run without erroring)
min_samples <- min(table(pca_data$group))

if (min_samples >= 3) {
  # Checking if the PC1 scores follow a bell curve for each group separately
  # We group by the condition, run the test, and pull out the p-value
  shapiro_results <- pca_data |>
    group_by(group) |>
    summarise(p_val = shapiro.test(PC1)$p.value)

  # If ALL groups have p > 0.05, we assume the data is normal enough
  is_normal <- all(shapiro_results$p_val > 0.05)
} else {
  # If we have tiny sample sizes, we can't prove normality, so we play it safe
  # and assume it's not normal (forcing the non-parametric test)
  is_normal <- FALSE
}

# The Decision Tree:
if (is_normal) {
  # Data looks normal -> T-test
  # (Parametric, more power if assumptions met)
  message("Data appears normal. Running T-test...")
  test_result <- t.test(PC1 ~ group, data = pca_data)

} else {
  # Data is skewed or sample size is small -> Wilcoxon
  # (Non-parametric, robust against outliers)
  message("Data is non-normal or small sample size. Running Wilcoxon test...")
  test_result <- wilcox.test(PC1 ~ group, data = pca_data)
}
...
create_volcano_plot <- function(shrunken_results, plot_title) {
  ...
  results_df <- results_df %>%
    # filter out the genes that don't have a symbol
    filter(!is.na(symbol)) |> 
  ...
  p <- ggplot(results_df, aes(x = log2FoldChange, y = -log10(padj))) +
    ...
    rasterise(geom_point(aes(color = significant), alpha = 0.6, shape = 16), dpi = 300) +
    ...
    #                 max.overlaps = 10.8, # Increase to allow more labels
    #                 size = 4,
    #                 box.padding = 0.4) +
...
p_li_contrast2
save_full_dataframe_to_excel(p_li_contrast2$data, "../mouse_data.xlsx", "Sup_Figure_9g_volcano_plot_e_faec_sugar", col_names = c(
  "log2FoldChange" = "log 2 Fold Change",
  "padj" = "Adjusted P value",
  "significant" = "Signficant Result"))
ggsave('../data/R38_DEG_volcano_li_contrast2.pdf', width = 6, height = 4)
```

```{r}
  results_df <- as.data.frame(shrunken_li_contrast2) %>%
    rownames_to_column("ensembl_version") %>%
    mutate(ensembl_id = str_remove(ensembl_version, "\\..*"))
...
SI_Serpina10
```
# gene symbol and normalized counts flat df

```{r}
# --- Step 1: Define a Function to Extract and Tidy Normalized Counts ---

# This function takes a dds object and its tissue name, then returns a tidy dataframe.
get_tidy_normalized_counts <- function(dds_object, tissue_name) {
  
  # Extract the normalized counts from the DESeqDataSet object
  normalized_counts_matrix <- counts(dds_object, normalized = TRUE)
  
  # Convert the matrix to a long-format (tidy) tibble
  tidy_counts <- as.data.frame(normalized_counts_matrix) %>%
    rownames_to_column("ensembl_version") %>%
    pivot_longer(
      cols = -ensembl_version,
      names_to = "sample_id",
      values_to = "normalized_count"
    ) %>%
    as_tibble()
  
  # Clean the Ensembl IDs by removing the version number (.1, .5, etc.)
  # This is crucial for matching with the annotation database.
  tidy_counts <- tidy_counts %>%
    mutate(ensembl_id = str_remove(ensembl_version, "\\..*"))
  
  # Add gene symbols by mapping the clean Ensembl IDs
  # It's efficient to map only the unique IDs to avoid redundant lookups.
  unique_ensembl_ids <- unique(tidy_counts$ensembl_id)
  gene_symbols <- mapIds(
    org.Mm.eg.db,
  ...
  # Create a small lookup table to join the symbols back to the main dataframe
  symbol_map <- tibble(ensembl_id = names(gene_symbols), symbol = gene_symbols)
  
  # Join the symbols and add the tissue annotation
  final_df <- tidy_counts %>%
    left_join(symbol_map, by = "ensembl_id") %>%
    mutate(tissue = tissue_name) %>%
    # Select and reorder columns for a clean final output
    select(ensembl_id, symbol, sample_id, normalized_count, tissue)
  
  return(final_df)
}


# --- Step 2: Apply the Function and Combine the Results ---

# Process the Large Intestine data
counts_li_tidy <- get_tidy_normalized_counts(dds_li, "Large_Intestine")

# Process the Small Intestine data
counts_si_tidy <- get_tidy_normalized_counts(dds_si, "Small_Intestine")

# Combine the two tidy dataframes into a single one
combined_counts_df <- bind_rows(counts_li_tidy, counts_si_tidy)

# --- Step 3: View the Results ---
unique_symbols <- combined_counts_df %>% distinct(symbol) 
```

```{r}
# why I didn't see the IP-10 gene  
# to see if I can find that gene in the counts before filtering
raw_counts_matrix_df <- raw_counts_matrix %>% 
  as.data.frame() %>% 
  rownames_to_column('ensembl_version') %>% 
  mutate(ensembl_id = str_remove(ensembl_version, "\\..*"))


unique_ensembl_ids <- unique(raw_counts_matrix_df$ensembl_id)
  gene_symbols <- mapIds(
    org.Mm.eg.db,
    keys = unique_ensembl_ids,
    column = "SYMBOL",
...
# The gene is there simpy too low to even pass the filtering
```
```{r}
# what are the gene counts of the other genes in the same family: Cxcl16, Cxcr4 and Cxcl12
three <- c('Cxcl16', 'Cxcr4' , 'Cxcl12')
three_gene_counts <- three %>% set_names(three) %>% 
  imap(function(gene_, name_){
    get_gene_counts(dds_li, gene_, "Large Intestine") %>% 
...
three_gene_counts_plot <- plot_grid(plotlist = three_gene_counts, nrow = 3)
ggsave('../data/R38_three_gene_counts_plot.pdf', height = 10)
```
...
sig_pw_lollipop_LI1 <- ggplot(plot_data_, aes(x = NES, y = pathway_clean, color = direction)) +
...
sig_pw_lollipop_LI1
save_full_dataframe_to_excel(plot_data_, "../mouse_data.xlsx", "Sup_Figure_9e_lollipop_gf_e_faec")
```
...
# newest version for the final RNA-seq figure

```{r}
p_rnaseq <- plot_grid(
  sig_pw_lollipop_LI1, pca_li_suc,
  p_li_contrast2,
  ncol = 1,
  rel_heights = c(1,.5,1)
)
...
ggsave('../data/R38_combined_RNA-seq.pdf',
      width = 8.5, height = 11, units = "in", device = cairo_pdf, plot = combined,
      dpi = 300)  
```
