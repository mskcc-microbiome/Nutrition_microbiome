...
```{r}
# --- Plotting an Enrichment Plot ---
# This plot shows how the genes in a specific set are distributed in your ranked list.

plotEnrichment(go_pathways[["GOBP_INTESTINAL_EPITHELIAL_CELL_DIFFERENTIATION"]], ranked_genes_li2) +
  labs(title = "Intestinal Epithelial Cell Differentiation")
```
...
sig_pw_bar_LI <- top_pathways %>%
  ...
  mutate(
    pathway_clean = pathway %>%
      str_replace("GOBP_", "") %>%       # Remove the "GOBP_" prefix.
      str_to_lower() %>%                # Convert the entire name to lowercase.
