...
save_plot("../data/R38_gsea_summary_figure.jpg", final_plot, base_width = 14, base_height = 12)
```
...
# This function will take a tidy dataframe and generate a heatmap.

generate_tissue_heatmap <- function(tidy_counts_df, tissue_name) {
  
  # Filter for the genes of interest
  plot_data_tidy <- tidy_counts_df %>%
    filter(gene %in% genes_to_plot)%>%
    filter(str_detect(treatment, "regular_water"))
  
  # Convert the tidy data to a matrix for pheatmap
  plot_matrix <- plot_data_tidy %>%
    dplyr::select(gene, sample_name, count) %>%
    pivot_wider(names_from = sample_name, values_from = count) %>%
    column_to_rownames("gene")
  
  # Create the annotation for the columns (samples)
  annotation_col <- plot_data_tidy %>%
    dplyr::select(sample_name, treatment) %>% # Can add other columns like 'water' if needed
    distinct() %>%
    column_to_rownames("sample_name")
  
  # Create the annotation for the rows (genes)
  ...
  # Order the matrix and annotations by Pathway for clean paneling
  annotation_row <- annotation_row %>% arrange(Pathway)
  plot_matrix <- plot_matrix[rownames(annotation_row), ]
  
  # Calculate the positions for the gaps between gene groups
  gaps_row <- table(annotation_row$Pathway) %>% cumsum() %>% head(-1)
  
  # Define custom colors
  ann_colors <- list(
    treatment = setNames(c("#A2E8E6", "#00BFC4"),
                         c("PBS__regular_water", "E. faecalis__regular_water")),
    Pathway = c(Proliferation = "#D95F02", `Immune Response` = "purple")
  )
  
  # Generate the heatmap
  pheatmap(
    plot_matrix,
    main = paste(tissue_name, "- Colonization Effect"),
    annotation_col = annotation_col,
    annotation_row = annotation_row,
    annotation_colors = ann_colors,
    scale = "row", # Scale genes to have mean 0 and SD 1 (Z-score)
    cluster_cols = F, # Cluster samples by similarity
    cluster_rows = FALSE, # Keep the defined pathway grouping
    gaps_row = gaps_row, # Add a visual gap between pathways
    show_rownames = F,
    fontsize_row = 10,
    filename = paste0("../data/gsea_leading_edge_heatmap_Colonization_", gsub(" ", "_", tissue_name), ".jpg"),
    width = each_width,
    height = each_height
  )
}

# --- Step 3: Generate the Two Heatmaps ---


# Generate the heatmap for the Small Intestine
generate_tissue_heatmap(all_counts_si, "Small Intestine")

# Generate the heatmap for the Large Intestine
generate_tissue_heatmap(all_counts_li, "Large Intestine")

####################################### sucrose effects

generate_tissue_heatmap_sucrose <- function(tidy_counts_df, tissue_name) {
  ...
  # Define custom colors
  ann_colors <- list(
    treatment = setNames(c("#00BFC4", "#F8766D"),
                         c("E. faecalis__regular_water", "E. faecalis__sucrose_water")),
    Pathway = c(Proliferation = "#D95F02", `Immune Response` = "purple")
  )
  ...
  pheatmap(
    plot_matrix,
    main = paste(tissue_name, "- Sucrose Effect"),
    annotation_col = annotation_col,
    annotation_row = annotation_row,
    annotation_colors = ann_colors,
    scale = "row", # Scale genes to have mean 0 and SD 1 (Z-score)
    ...
    show_rownames = F,
    fontsize_row = 10,
    filename = paste0("../data/gsea_leading_edge_heatmap_sucrose_", gsub(" ", "_", tissue_name), ".jpg"),
    width = each_width,
    height = each_height
  )
}
...
# Generate the heatmap for the Small Intestine
generate_tissue_heatmap_sucrose(all_counts_si, "Small Intestine")

# Generate the heatmap for the Large Intestine
generate_tissue_heatmap_sucrose(all_counts_li, "Large Intestine")
```
