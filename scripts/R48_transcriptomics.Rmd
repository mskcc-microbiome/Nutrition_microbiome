...
day1_plot_data <- day1_comp_data |>
  mutate(
    ...
    sample_id = factor(sample_id, levels = c("MR12", "MR14", "MR15", "MR13", "MR4", "MR5", "MR6", "MR7"))
  ) |>
...
focused_composition_plot <- ggplot(day1_plot_data, aes(x = sample_id, y = relative_abundance, fill = g_s_highlight)) +
  geom_col(position = "fill") +
  ...
  labs(
    title = "Composition of Day 1 Sucrose Groups",
    x = "Sample",
    y = "Relative Abundance",
    fill = "Taxa"
  ) +
...
# Print the new plot.
print(focused_composition_plot)
```
...
heatmap_plot <- ggplot(heatmap_data, aes(x = sample_id, y = gene_label, fill = log2_tpm)) +
  ...
  labs(
    ...
    subtitle = "Day 3: Biapenem + Sucrose vs. PBS + Sucrose",
    x = "Sample",
    y = "",
    fill = "Log2(RNA TPM + 1)"
  ) +
