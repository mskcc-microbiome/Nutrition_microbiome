...
meds_expanded %>%  write_csv('../data/R32_meds_expanded_all_medication_classified_for_each_day.csv')
```
```{r}
...
# actually only P108 is most obvious
trend <- c('P108')
...
# look at what medication this patient had exactly 
patient_med_data <- meds_updated %>% 
  filter(pid == 'P108') %>% 
  distinct() %>% 
  filter(startday %in% 10:21 & stopday %in% 10:21) %>% 
  filter(drug_category_for_this_study == 'broad_spectrum')


# ---  Create a summary dataframe for placing the annotations ---
annotation_data <- patient_abx_data %>%
  group_by(pid) %>%
  # Find the middle day of the exposure period to center the text
  summarise(x_position = mean(range(day)), .groups = "drop")
```
...
ggplot() +
  ...
  geom_rect(
   ...
   # --- Add the text annotation for the antibiotic ---
  geom_text(
    data = annotation_data,
    aes(x = x_position, y = Inf, label = "piperacillin-tazobactam"),
    vjust = 1.5,        # Nudge the text down from the top edge
    size = 3.5,         # Adjust size for readability
    color = "#FF0000",
    fontface = "italic" # Make it stand out a bit
  ) +
  # Add the line and points for microbiome diversity
  geom_line(
    data = patient_diversity_data,
...
ggsave('../data/R32_rebound_patient.jpg', width = 5, height = 3)
```
