...
```{r}
all_food_vars <- meta_ASVs %>% select(starts_with('fg')) %>% colnames()
...
fit_brm_model <- function(asv_outcome_name) {
  ...
  model_fit <- brm(
    ...
    data = meta_ASVs,
    prior = priors,
    warmup = 500, iter = 1000, # Reduced for example
    chains = 2, cores = 1, # `cores = 1` is important for running in parallel
    seed = 123,
    silent = 2,
    backend = "cmdstanr" # Recommended if you have cmdstanr installed
  )
  
  # Tidy the results and return them
