...
post_res %>%  write_csv('../data/173_div_model_fg_post_interaction.csv')
```
```{r}
# to see the median and the 95% CI of the coeff
post_res <-  read_csv('../data/173_div_model_fg_post_interaction.csv') |> gather()

post_res %>% filter(str_detect(key, '_e$')) %>% 
  group_by(key) %>% 
  tidybayes::median_qi(value , .width = c( .95)) %>% 
  arrange(.lower) |> 
