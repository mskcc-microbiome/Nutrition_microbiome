...
```{r}
model_fit_original <- brm(
  formula = formula,
  data = meta |> mutate(across(starts_with("fg_"), ~ .x * 100)),
  prior = priors,
    warmup = 1000, iter = 3000,
    chains = 4, cores = 4,
    seed = 123,
...
dat_conditional_mod_entero <- conditional_effects(model_fit_original, surface = T)
...
ggsave('../data//E19_conditional9_entero_asv1_171.pdf',
      width = 8.5, height = 11, units = "in", device = 'pdf', plot = combined, dpi = 300)  
```
...
```{r}
# calculate the trapezoidal auc from value
...
auc_results <- newest %>%
  ...
  summarize(
    ...
    auc_value = AUC(x = Day, y = CFUs_per_GramStool),
...
f3_AUC
save_ggplot_data_to_excel(f3_AUC, auc_results, "../mouse_data.xlsx", "Figure_4c_trapezoidal_auc", col_names = c(
  "auc_value" = "Trapezoidal AUC Value",
...
deltas %>% 
  filter(str_detect(grp, 'biapenem') & Day != '1') %>% 
  droplevels() %>% 
  split(.$Day) %>% 
  map(function(df){
...
f3_days
save_ggplot_data_to_excel(f3_days, plotdf, "../mouse_data.xlsx", "Figure_4c_indiv_days_data", col_names = c(
  "xvar" = "Treatment + Day",
