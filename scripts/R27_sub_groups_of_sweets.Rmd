...
## first 2 digits exposure samples and patients 
...
exposure_number2 <- final_model_data %>% 
  pivot_longer(
    cols = starts_with("avg_intake_"),
    names_to = "key",
    values_to = "intake_value"
  ) %>%
  # Filter the reshaped data to only include rows where intake was recorded.
  filter(intake_value > 0) %>%
  # Group the data by the new category column.
  group_by(key) %>%
  # Calculate the two summary statistics for each group.
  # 'n()' counts the total number of rows in the group (the non-zero observations).
  # 'n_distinct(pid)' counts the unique patient IDs in that group.
  summarise(
    non_zero_rows = n(),
    unique_pids = n_distinct(pid)
  ) %>% 
  # Clean up the names for the final table
  inner_join(level2,  by = "key") %>% 
...
```{r}
library(ggtree)
# 1. Define the Tree Structure in Newick format
# This text format describes the parent-child relationships from your PDF.
tree_text <- "((('91: Sugars and sweets'), ('92: Nonalcoholic beverages'), ('94: Water noncarbonated'), ('95: Formulated nutrition beverages etc.'))'9: Sugars, Sweets, and Beverages');"

# Read the text to create a formal tree object.
phylo_tree <- read.tree(text = tree_text)

# 2. Create the Annotation Data
# Recreate the summary data from your screenshot and add a 'label' column
# that exactly matches the tip labels in the tree.
annotation_data <- exposure_number2 %>%
  # Create the text that will be displayed on the plot.
  mutate(
    exposure_text = paste("Samples:", non_zero_rows, " Patients:", unique_pids),
    # Map the summary data to the exact tip labels from the tree.
    label = case_when(
      food_category == "Sugars_and_sweets" ~ "'91: Sugars and sweets'",
      food_category == "Nonalcoholic_beverages" ~ "'92: Nonalcoholic beverages'",
      food_category == "Water" ~ "'94: Water noncarbonated'",
      food_category == "Formulated_beverages" ~ "'95: Formulated nutrition beverages etc.'"
    )
  )

# 3. Create the Plot
# Start by plotting the basic tree.
tree_plot <- ggtree(phylo_tree)

# Explicitly join the annotation data to the tree's internal data.
tree_plot$data <- tree_plot$data %>%
  left_join(annotation_data, by = "label") %>%
  # Create a new, combined label that includes the exposure data for the tips.
  mutate(
    combined_label = if_else(
      isTip & !is.na(exposure_text),
      paste(label, exposure_text, sep = " \nExposure number:      "),
      label # Keep the original label for internal nodes
    )
  )

# Now, build the final plot using the new combined label.
annotated_tree_plot <- tree_plot +
  # Use the new 'combined_label' for the tip text.
  geom_tiplab(aes(label = combined_label), align = TRUE, size = 3.5) +
  # Add the label for the internal node (the root in this case).
  geom_nodelab(
    aes(label = combined_label, subset = !isTip), # Label only non-tip nodes
    hjust = 1.2, # Position the label to the left of the node
    size = 4
    ) +
  # Set the plot limits to make enough space for the new text.
  xlim(-28, 49)


# Print the final plot.
print(annotated_tree_plot)
   
ggsave('../data/R27_g9_tree.pdf', width = 8.5)
```
```{r}
# "can you please provide the median grams of exposure for each of these subgroups? I'm wondering if that might help explain why there is no significance in group 91 (candy)"
boxplot_plot <- final_model_data %>%
  # Reshape the data from a "wide" format to a "long" format.
  # This is the standard tidyverse way to plot multiple columns.
  pivot_longer(
    # This selection logic is the core of the request:
    # 1. Select columns that contain "avg_intake"
    # 2. AND (&) exclude (!) columns that contain "fg"
    cols = contains("avg_intake") & !contains("fg"),

    # 'names_to' creates a new column to store the original column names
    names_to = "intake_category",
    # 'values_to' creates a new column to store the values from those columns
    values_to = "value"
  ) %>%
  mutate(value = value * 100) |> 

  # Now, create the plot using the new long-format data
  # Map categories to x-axis and values to y-axis
  ggplot(aes(x = intake_category, y = value, fill = intake_category)) +
  ...
  # No faceting is needed; this places all boxplots on the same row.

  # Add informative labels and a clean theme
  labs(
    title = "Boxplot Distribution of Average Intakes",
    x = "Intake Category",
    y = "Average Intake Value"
  ) +
  theme_minimal() +
  ...
  theme(
    ...
    # Hide the legend since the x-axis is already labeled
    legend.position = "none"
  ) + scale_y_sqrt()
...
ggsave('../data/R27_box_4_subsweets.jpg', width = 6, height = 8)

dtb_94 <- dtb %>% 
  distinct(Food_NSC, Food_code, description) %>% 
  mutate(Food_code = as.character(Food_code)) %>% 
  filter(str_detect(Food_code, '^94'))
```
...
subsweets_plot <-  plot_grid(annotated_tree_plot,
                          bottom,
...
combined <- plot_grid(
  title, subsweets_plot,
  ncol = 1,
  # rel_heights values control vertical title margins
  rel_heights = c(0.1, 1)
) +theme(plot.margin = unit(c(1,1,1,1), "cm"))

ggsave('../data/E04e_R27_subgroup_sweets_FNDDS.pdf',
      width = 4, height = 6, units = "in", device = 'pdf', plot = plot_subsweets,
      dpi = 300)  
```
