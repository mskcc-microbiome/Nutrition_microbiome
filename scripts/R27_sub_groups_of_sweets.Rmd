...
ggplot(plot_data_all, aes(x = total + pseudocount, y = sugar + pseudocount)) +
  # Add the points for each observation. `alpha` adds transparency.
  geom_point(alpha = 0.5, aes(color = food_category)) +
  # Add a regression line to show the general trend.
  geom_smooth(method = "lm", se = FALSE, color = "black", linetype = "dashed") +
  # Add a 1-to-1 line. Points above this line are impossible (more sugar than total).
  # Points on this line are pure sugar. Points below show the proportion.
  geom_abline(intercept = 0, slope = 1, color = "red", linewidth = 1) +
  
  # Use a log scale for both axes to better visualize the data,
  # which is often skewed towards zero.
  ...
  # Add informative labels
  labs(
    title = "Relationship Between Total Intake and Sugar Content",
    subtitle = "Each point is a patient-day. The red line indicates 100% sugar content.",
    x = "Total Dehydrated Weight (grams, log scale)",
    y = "Sugar Weight (grams, log scale)"
  ) +
  # Use a clean theme
  theme_bw() +
  theme(legend.position = "none") # Hide legend as facets are clear
```
