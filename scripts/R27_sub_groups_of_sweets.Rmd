...
nodes_l2 <- nodes_l3 %>%
  ...
  summarise(y = mean(y)) %>%
  rename(code = parent) %>%
  mutate(x = 2, parent = '9') %>% select(colnames(nodes_l3))

# Calculate position for the root node (level 1) by centering it over its children
nodes_l1 <- nodes_l2 %>%
  summarise(y = mean(y)) %>%
  mutate(x = 1, code = '9', parent = NA)%>% select(colnames(nodes_l3))
...
all_nodes <- bind_rows(nodes_l1, nodes_l2, nodes_l3) %>%
  ...
  left_join(select(code_to_desc, code, label), by = c("code" = "code"))
...
ggplot() +
  ...
  geom_segment(data = edges, aes(x = x, y = y, xend = xend + 0.05, yend = y), color = "gray50") +
  # Draw the vertical segments of the tree
  geom_segment(data = edges, aes(x = xend + 0.05, y = y, xend = xend + 0.05, yend = yend), color = "gray50") +

  # Draw the points for each node
  geom_point(data = all_nodes, aes(x = x, y = y, color = factor(x)), size = 4) +
  
  # Draw the text labels, now rotated vertically
  geom_text(
    data = all_nodes,
    aes(x = x, y = y, label = label),
    angle = 0, # Rotate labels 90 degrees
    vjust = -0.7, # Adjust vertical justification to move text above the point
  ...
  scale_color_manual(values = c("1" = "#073B4C", "2" = "#118AB2", "3" = "#06D6A0"), guide = "none") +
  
  # Reverse the y-axis to have the root at the top and expand x-axis for labels
  scale_y_reverse() +
