...
# Function to recursively build the Newick string
build_newick <- function(parent_code, data, all_data) {
  children <- data %>% filter(f2_code == parent_code)
  if (nrow(children) == 0) {
    return(as.character(parent_code))
  }
  
  child_strings <- children$f3_code %>%
    map_chr(~ as.character(.x))
  
  paste0("(", paste(child_strings, collapse = ","), ")", parent_code)
}

# Build the tree structure for each level 2 group
level2_groups <- fndds_structure_table %>% distinct(f2_code) %>% pull()
level2_newick <- map_chr(level2_groups, ~build_newick(.x, fndds_structure_table))

# Combine into the final Newick string with the root node "9"
final_newick_string <- paste0("(", paste(level2_newick, collapse = ","), ")9;")

# --- Step 3: Create a 'phylo' Object and Plot with ggtree ---
# The 'read.tree' function from the 'ape' package parses the Newick string.
tree <- read.tree(text = final_newick_string)

# --- Step 4: Create an Annotation Data Frame ---
# This data frame will map the node/tip labels (which are just numbers/codes)
# to the full descriptive labels we want to plot.
annotation_df <- as_tibble(tree) %>% # Convert the tree structure to a table to get node numbers
  select(node, label) %>% # We only need the node number and its original label (the code)
  left_join(code_to_desc, by = c("label" = "code")) %>% # Join with our descriptions
  # The final label is in the 'label.y' column from the join.
  # We also handle the root node which might not have a label from the join.
  mutate(full_label = coalesce(label.y, label)) %>%
  select(node, full_label) # Keep the node number and the final descriptive label


# --- Step 5: Create the Tree Visualization ---
# The '%<+%' operator joins our annotation data to the tree plot data.
ggtree(tree, layout = 'rectangular') %<+% annotation_df +
  # Add labels to the tips and internal nodes of the tree.
  # 'label=full_label' now works because 'full_label' is in the joined data.
  # geom_label2 is a ggtree function that is often more flexible for tree labels.
  geom_label2(aes(label = full_label, subset = !is.na(full_label)), 
              hjust = -0.05, 
              label.size = 0, 
              alpha = 0.6, 
              fill = "skyblue") +
  # Add some space on the x-axis for the labels
  xlim(0, 4) +
  # Add a title
