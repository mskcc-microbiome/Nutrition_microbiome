...
stratified_exposure |> select(sampleid, exposure_type) |> write_csv('../data/R41_stool_sample_abx_exposure_type_4.csv') 
```
...
# Enterococcus as outcome 

```{r}
formula_string_Enterococcus <- paste(
  "asv_1_CLR ~ 0 + intensity + exposure_type + TPN + EN +",
  paste(interaction_terms, collapse = " + "),
  "+ (1 | pid) + (1 | timebin)"
)

# Convert the string to a formula object
formula_Enterococcus <- brms::bf(as.formula(formula_string_Enterococcus))
...
# Fit the model
model_fit_Enterococcus <- brm(
  formula = formula_Enterococcus,
  data = meta,
  prior = priors_Enterococcus,
  warmup = 1000, iter = 3000,
  chains = 4, cores = 4,
  seed = 123,
  silent = 2,
...
results_df_Enterococcus <- tidy(model_fit_Enterococcus, conf.int = TRUE) %>% 
  mutate(conf.low = round(conf.low, 2),
         conf.high = round(conf.high, 2))
# The main way brms speeds things up is by running each of the model's independent simulations (the "chains") on a separate CPU core. Think of it like having multiple checkout lanes at a grocery store; if you have four lanes open for four customers, things move much faster than having just one lane open.
# The most effective way to speed up your model is to run more chains in parallel. A good rule of thumb is to set the number of chains equal to the number of cores you want to use. Running at least 4 chains is also generally recommended for checking that the model has converged properly.
# 8 chains : 15s, 2 chains: 12s, 4 chains : 13.5s
# For models that are already very fast, the time cost of setting up parallel chains can be greater than the time saved. Your 2-chain model is faster because the overhead is lower. However, you should still use 4 chains, as it is the gold standard for verifying that your model has run correctly, and that diagnostic confidence is worth the extra 1.5 seconds.
```

```{r}
cleaned_effects_Enterococcus <- results_df_Enterococcus %>%
  # Keep only the fixed effects
  filter(effect == "fixed") %>%
  # Create a new column to distinguish main effects from interactions
  mutate(
...
shading_df <- cleaned_effects_Enterococcus %>%
  mutate(y_numeric = as.numeric(clean_term)) %>%
  filter(str_detect(clean_term, "\\*")) # Filter for interaction terms

plot_stratify_ASV1 <- ggplot(cleaned_effects_Enterococcus, aes(x = estimate, y = clean_term)) +
  geom_rect(
    data = shading_df,
    aes(ymin = y_numeric - 0.5, ymax = y_numeric + 0.5, xmin = -Inf, xmax = Inf),
    fill = "#FBEADC", # A light orange/peach color like the example
  ...
  labs(
    x = "CLR(ASV_1) change",
    title = "Predictor effects on ASV 1 (E. faecium),\nexcluding samples exposed to C. diff and BSI treatment",
    y = ''
  ) +
  theme_classic(base_size = 11) +
  theme(legend.position = "none", axis.text.y = element_markdown())
...
final_plot <- plot_grid( table_tally ,  plot_stratify_ASV1 , 
                nrow  =2,
                 rel_heights = c(.9,2),
                labels = 'auto',
                align = 'vh', axis = 'lrtb')
...
ggsave('../data/R41_ASV1__remove_2_abx_stratification.pdf',
      width = 8.5, height = 11, units = "in", device = 'pdf', 
      dpi = 300)
```
