...
table_data <- stratified_tally_detailed %>%
  ...
  mutate(exposure_type = factor(exposure_type, levels = c("Empiric only", "No broad-spectrum exposure"))) %>%
  arrange(exposure_type)


# --- Table Creation and Styling ---
# Use the gt package to build and format the table from the prepared data.
exposure_table_tally <- table_data %>%
  gt() %>%
  ...
  cols_align(
    align = "center",
    columns = c(number_of_samples, number_of_patients)
  ) %>%
...
```{r}
table_tally <- ggdraw() + draw_image("../data/R41_number_tally.png", scale = 1)


final_plot <- plot_grid( table_tally , sweets_raw  , plot_stratify , 
                nrow  =3,
                 rel_heights = c(0.9,1.3,2),
                labels = 'auto',
                align = 'vh', axis = 'lrtb')
