...
run_deseq_with_batch_correction <- function(full_counts, full_meta, target_tissue, analysis_type) {
  ...
  if (analysis_type == "Colonization") {
    ...
    # This design is confounded in batch2 (no PBS samples).
    # Therefore, we ONLY use batch1 samples where the design is complete.
    meta_subset <- full_meta %>% 
      filter(tissue == target_tissue, 
             water == "regular_water",
             batch == "batch1") # <-- Added this filter
    
    # Since we are only using one batch, 'batch' is removed from the design formula.
    ...
    # This design is valid across batches.
    meta_subset <- full_meta %>% 
      filter(tissue == target_tissue, 
             treatment == "E. faecalis")
    
    # Include 'batch' in the design to correct for it.
  ...
  # Check if any samples are left after filtering
  if (nrow(meta_subset) == 0) {
    stop(paste("No samples found for tissue:", target_tissue, "and analysis:", analysis_type))
  }
  
  # Subset the counts matrix to match the selected metadata
  # Ensure sample IDs are characters for matching
  counts_subset <- full_counts[, as.character(meta_subset$sample_id)]
  
  # Ensure row and column names of colData and countData match
  rownames(meta_subset) <- meta_subset$sample_id
  counts_subset <- counts_subset[, rownames(meta_subset)]
  
  # Create the DESeqDataSet object with the appropriate design
  ...
  # Using resultsNames(dds) to ensure contrast is valid
  message("Running contrast: ", paste(contrast_info, collapse = " "))
  results <- results(dds, contrast = contrast_info)
  
  # Apply LFC shrinkage for more accurate fold changes
  # 'contrast' is used for 'ashr' shrinkage
...
```{r}
meta_subset <- meta_table %>% filter(tissue == "small_intestine", water == "regular_water")
