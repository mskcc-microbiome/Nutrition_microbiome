...
# 2. Correct for batch effect
#    We use limma::removeBatchEffect on the variance-stabilized counts
#    The 'batch' argument needs the batch vector from your coldata
...
plot_data <- batch_corrected_counts %>%
  as.data.frame() %>%
  rownames_to_column("ensembl_version") %>%
...
```{r}
# 4. Create the faceted boxplot using the batch-corrected counts
ggplot(plot_data, aes(x = treatment, y = batch_corrected_count, color = treatment)) +
  ...
  labs(
    title = "Batch-Corrected Counts of B-Cell Signature Genes",
    subtitle = "SI: Colonization Effect (VST + removeBatchEffect)",
...
ggsave("../data/R49_bcell_gene_counts_boxplot_batch_corrected.jpg", width = 12, height = 9, dpi = 300)
```
```{r}
...
if (length(inflammation_genes_for_heatmap) > 0) {
  ...
  #   # Try to match common names, otherwise just assign
  #   if (all(c("E. faecalis", "PBS") %in% condition_levels)) {
  #     names(cond_colors) <- c("E. faecalis", "PBS")
  #   } else {
  #     names(cond_colors) <- condition_levels
  #   }
  # } else {
