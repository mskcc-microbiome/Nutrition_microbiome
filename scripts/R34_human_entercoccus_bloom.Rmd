...
```{r}
# Calculate the median and IQR for duration_days, grouped by the threshold label
summary_table <- combined_summary %>%
  group_by(threshold_label) %>%
  summarise(
    # Calculate the 25th percentile and convert to an integer
    q25_duration = as.integer(quantile(duration_days, probs = 0.25, na.rm = TRUE)),
    # Calculate the median (50th percentile) and convert to an integer
    median_duration = as.integer(median(duration_days, na.rm = TRUE)),
    # Calculate the 75th percentile and convert to an integer
    q75_duration = as.integer(quantile(duration_days, probs = 0.75, na.rm = TRUE)),
    .groups = 'drop' # Drop the grouping for a clean final table
  )
...
```{r}
final_plot <-  plot_grid( duration_days_estimate , 
...
combined <- plot_grid(
  ...
  rel_heights = c(0.1, 1)
) +theme(plot.margin = unit(c(1,6,16,1), "cm"))
