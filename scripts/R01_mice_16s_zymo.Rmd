...
```{r}
# the same version as above but with p value spelled out
meta %>% 
  mutate(day = factor(day)) %>% 
  arrange(abx_treatment,desc(diet_treatment),  day) %>% 
  mutate(xvar = str_glue('{abx_treatment}__{diet_treatment}__{day}')) %>% 
  ...
  stat_compare_means(comparisons = list(
    ...
    label= "p.value", method= 'wilcox.test',tip.length = 0.04,exact=T,correct=T) +
  scale_x_discrete(labels=rep(c(0,3,6),4)) + 
  scale_y_sqrt() +
  theme_light()
```
...
beta_4 <- beta_all %>% 
  mutate(experiment_no = str_glue('Expe. {experiment_no}'),
         day  = str_glue('D{day}')) %>% 
  ggscatter(x = 'V1', y = 'V2', color = 'abx_treatment', shape = 'diet_treatment', alpha = 0.9) +
  facet_grid( .~  day) +
  theme_light() +
  scale_color_manual(values = c('#C93312', '#899DA4'), name = 'abx_treatment') +
...
abx_asvs_relab_plots <- abx_asvs_relab %>% 
  ...
  ggboxplot(x = 'day', y = 'relab',outlier.shape = NA,
   ...
   #scale_color_manual(values = c('gray76','#ffbcdc','gray32','deeppink2')) +
  stat_compare_means(comparisons = list(c('0','3'), c('0','6')),label= "p.value", method= 'wilcox.test',paired = T, vjust = 0.4,tip.length = 0.03,exact=F) +
  #scale_x_discrete(labels=rep(c(0,3,6),4)) + 
...
top_2 <- plot_grid(alpha_healthy_days, beta_4_plot, nrow = 1, align = 'vh', axis = 'lrtb', rel_widths = c(1,1.5), labels = c('A','B'))
