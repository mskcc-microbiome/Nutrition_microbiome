...
sugar_asvs_relab %>% 
  ...
  ggboxplot(x = 'day', y = 'relab',
             xlab = '', ylab = ' relative abundance',, add.params = list(alpha = 0.5, shape = 16),
            title = str_glue('{name_}'),
            #color  = 'grp',
            width = 0.6)+
       geom_point(aes(group = mouse_no),alpha=0.3) +
      geom_line(aes(group = mouse_no),alpha=0.3) +
   #scale_color_manual(values = c('gray76','#ffbcdc','gray32','deeppink2')) +
  stat_compare_means(comparisons = list(c('vehicle','sucrose')),label= "p.value", method= 'wilcox.test',tip.length = 0.04,exact=F) +
  #scale_x_discrete(labels=rep(c(0,3,6),4)) + 
      facet_grid(. ~ diet_treatment) +
  theme(axis.text =  element_text(size = 10, angle = 45, hjust = 1),axis.title=element_text(size=10),legend.position = 'none',aspect.ratio=1/1)
  })
