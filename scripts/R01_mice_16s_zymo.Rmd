...
beta_4 <- beta_all %>% 
  mutate(experiment_no = str_glue('Expe. {experiment_no}'),
         day  = str_glue('D{day}')) %>% 
  ggscatter(x = 'V1', y = 'V2', color = 'treatment',  alpha = 0.9, shape = 16) +
  facet_grid( experiment_no~  day) +
  theme_light() +
  scale_color_manual(values = c('gray76','#ffbcdc','gray32','deeppink2'), name = '') +
...
sugar_asvs_relab_plots <- sugar_asvs_relab %>% 
  ...
  ggboxplot(x = 'day', y = 'relab',outlier.shape = NA,
   ...
   #scale_color_manual(values = c('gray76','#ffbcdc','gray32','deeppink2')) +
  stat_compare_means(comparisons = list(c('0','3')),label= "p.value", method= 'wilcox.test',paired = T, tip.length = 0.04,exact=F) +
  #scale_x_discrete(labels=rep(c(0,3,6),4)) + 
...
```{r}
# why some mice seem to not match
# there are two experiments!
test <- sugar_asvs_relab %>% 
  filter(feature == 'seq1.g__Lactobacillus.s__johnsonii') %>% 
  filter(abx_treatment == 'biapenem') %>% 
  select( relab, mouse_no, day, diet_treatment, experiment_no) %>% 
  split(.$diet_treatment) %>% 
  map(~ spread(data= . ,'day','relab') %>%
...
abx_asvs_relab_plots <- abx_asvs_relab %>% 
  ...
  ggboxplot(x = 'day', y = 'relab',outlier.shape = NA,
      ...
      geom_line(aes(group = u_mouse_id),alpha=0.3) +
   #scale_color_manual(values = c('gray76','#ffbcdc','gray32','deeppink2')) +
  stat_compare_means(comparisons = list(c('0','3'), c('0','6')),label= "p.value", method= 'wilcox.test',paired = T, tip.length = 0.04,exact=F) +
  #scale_x_discrete(labels=rep(c(0,3,6),4)) + 
...
top_2 <- plot_grid(alpha_healthy_days, beta_4_plot, nrow = 1, align = 'vh', axis = 'lrtb', rel_widths = c(1,1.1), labels = c('A','B'))
top_2
```
...
p1 <- plot_grid(top_2, abx_top, abx_relabs, nrow = 3)
...
sugars <- plot_grid(sugar_top_plot,sugar_relabs, nrow = 2 , align = 'vh', axis = 'lrtb', rel_heights = c(1,2))
