...
all_res %>%
  ...
  geom_text_repel(aes(label = ifelse(qval < 0.05 & coef > 1, cleaned_name, "")), color = 'red') +
  geom_text_repel(aes(label = ifelse(qval < 0.05 & coef < -1, cleaned_name, "")), color = 'blue') +
  theme(aspect.ratio = 1/2, legend.position = 'none')

ggsave('../data/R01_maaslin2_abx+sucrose_d3_volcano.pdf', width = 7, height = 5)
ggsave('../data/R01_maaslin2_abx+sucrose_d3_volcano.jpg', width = 7, height = 5)
```
...
all_res %>%
  ...
  geom_vline(xintercept =  c(1, -1), color = 'gray', linetype = 'dashed') + 
  geom_hline(yintercept =  -log10(0.05), color = 'gray', linetype = 'dashed') + 
  geom_text_repel(aes(label = ifelse(qval < 0.05 & abs(coef) > 1, cleaned_name, "")), color = 'red') +
  geom_text_repel(aes(label = ifelse(qval < 0.05 & coef < -1, cleaned_name, "")), color = 'blue') +
  theme(aspect.ratio = 1/2, legend.position = 'none')

ggsave('../data/R01_maaslin2_abx+sucrose_d6_volcano.pdf', width = 7, height = 5)
ggsave('../data/R01_maaslin2_abx+sucrose_d6_volcano.jpg', width = 7, height = 5)
```
...
# to use the volcano plot to highlight the results on day 3
all_res <- read_tsv('../data/R01_maaslin2_output_ALL_grp4_family/all_results.tsv') 
...
all_res %>%
  filter(metadata == 'treatmentbiapenem_sucrose:day3') %>% 
  mutate(sig_ = if_else(qval < 0.05 & abs(coef) > 1, T, F)) %>% 
  mutate(neglog10_q = -log10(qval)) %>%
  ggscatter(x = 'coef', y = 'neglog10_q', color = 'sig_', alpha = 0.5, shape = 16, 
            xlab = 'Maaslin2 model coefficient', ylab = '- log10(q value)',
            title = 'Differential abundance between day 0 and day 3\nin abx + sucrose VS abx + vehicle\nat Family level') +
  ...
  geom_vline(xintercept =  c(1, -1), color = 'gray', linetype = 'dashed') + 
  geom_hline(yintercept =  -log10(0.05), color = 'gray', linetype = 'dashed') + 
  geom_text_repel(aes(label = ifelse(qval < 0.05 & abs(coef) > 1, feature, "")), color = 'red') +
  theme(aspect.ratio = 1/2, legend.position = 'none')

ggsave('../data/R01_maaslin2_abx+sucrose_d3_volcano_family.pdf', width = 7, height = 5)
ggsave('../data/R01_maaslin2_abx+sucrose_d3_volcano_family.jpg', width = 7, height = 5)
```
...
ggsave('../data/R01_maaslin2_abx+sucrose_d6_volcano_family.pdf', width = 7, height = 5)
ggsave('../data/R01_maaslin2_abx+sucrose_d6_volcano_family.jpg', width = 7, height = 5)
```
