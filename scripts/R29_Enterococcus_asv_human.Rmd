...
## abundance and prevelance of the two 
...
# Now, create the raincloud plot
relab_rain <- ggplot(plot_data, aes(x = asv_key, y = count_relative)) +
  
  # The "Cloud": a half-violin plot to show the distribution's shape
  ggdist::stat_halfeye(
    adjust = 0.5, 
    width = 0.6, 
    .width = 0, 
  ...
  # A traditional boxplot to show summary stats (median, quartiles)
  # geom_boxplot(
  #   width = 0.12, 
  #   outlier.shape = NA # outliers will be shown by the jitter plot
  # ) +
  # 
  # The "Rain": show all the individual data points
  gghalves::geom_half_point(
    side = "l", # show points on the left side
    range_scale = 0.4, 
    alpha = 0.2
  ) +
  
  # Add a point for the mean, making it stand out
  stat_summary(fun = mean, geom = "point", shape = 23,  # Here, we map the color aesthetic to a descriptive string
    aes(color = "Mean"),  size = 3, fill = "white") +
  # This new scale manually defines the color for our "Mean" aesthetic
  scale_color_manual(name = "Statistic", values = c("Mean" = "black")) +
...
p2 <- ggplot(plot_data_bars, aes(x = prevalence, y = asv_key)) +
  geom_col(aes(fill = prevalence), alpha = 0.8) +
...
p_asv_level <- plot_grid(relab_rain, p2, 
...
# A dedicated plot just for our data-driven labels.
p_labels <- ggplot(summary_data, aes(x = 0, y = species_ordered)) +
  # Use geom_text to create the labels, sizing them by mean abundance
  geom_text(aes(label = species_label, size = mean_abundance), hjust = 0) +
  # This special scale works with reorder_within
  scale_y_reordered() +
  # Control the range of font sizes for a nice visual effect
  scale_size_continuous(range = c(2.5, 8)) +
  facet_wrap(~ asv, ncol = 1, scales = "free_y") +
  # Make everything except the text invisible
  theme_void() +
  theme(
    strip.text = element_blank(), # Hide facet titles, they'll be on the other plots
    legend.position = "none"
  )
...
p_abundance <- ggplot(plot_data_sorted, aes(x = abundance, y = species_ordered)) +
  ...
  legend.position = "none",
    ...
    axis.text.y = element_blank(), # Hide the default axis text
...
p_species_level <- plot_grid(p_labels, p_abundance, p2, align = 'h', # Align horizontally
          axis = "tb",
          nrow = 1,
          rel_widths = c(0.3, 1.5, 1))
```
...
final_plot <- plot_grid(
  p_asv_level, 
  p_species_level, 
  nrow = 2,          # Arrange them in two rows (vertically)
  align = 'hv',       # Align them horizontally
  rel_heights = c(1, 1.5) # Give the bottom plot 1.5x the height of the top one
)
