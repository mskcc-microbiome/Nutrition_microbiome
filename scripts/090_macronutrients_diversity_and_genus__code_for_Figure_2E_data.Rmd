...
p_corrplot <- ggplot(plot_data_lower_tri, aes(x = measure1, y = measure2, fill = r)) +
  ...
  scale_fill_gradient2(
    ...
    high = "#7F0000",
    midpoint = 0,
    limit = c(-1, 1), # Force the scale to be from -1 to 1
    name = "r"
  ) +
...
```{r}
full <- read_csv('../data/090_all_samples1009_meta_p2d_fg9_dietall_genera90_pid.csv')
macro_cols_p2d <- full %>% select(starts_with('ave')) %>% select(-ave_cal) |> colnames()
...
rmcorr_results_p2d <- pairs |>
  map(\(pair) {
    ...
    result <- rmcorr(
      ...
      measure1 = m1_name,
      measure2 = m2_name,
      dataset = full |> mutate(pid = factor(pid))
    )
...
level_order <- rev(c(
    ...
    "abx * Fiber", "Fiber",
    "abx * Fat", "Fat",
    "abx * Carbohydrates", "Carbohydrates",
    "abx * Protein", "Protein"
))
...
cleaned_effects <- results_df %>%
  ...
  mutate(
    ...
    clean_term = case_when(
      ...
      term == "empiricalTRUE:ave_carb" ~ "abx * Carbohydrates",
      term == "empiricalTRUE:ave_fat" ~ "abx * Fat",
      term == "empiricalTRUE:ave_fiber" ~ "abx * Fiber",
      term == "empiricalTRUE:ave_Protein" ~ "abx * Protein",
      term == "empiricalTRUE:ave_Sugars" ~ "abx * Sugars",
      
      # Nutrient-only terms (the 'FALSE' condition)
      # These seem to be the main nutrient effects without the interaction.
      term == "empiricalFALSE:ave_carb" ~ "Carbohydrates",
      term == "empiricalFALSE:ave_fat" ~ "Fat",
      term == "empiricalFALSE:ave_fiber" ~ "Fiber",
      term == "empiricalFALSE:ave_Protein" ~ "Protein",
      term == "empiricalFALSE:ave_Sugars" ~ "Sugars",
