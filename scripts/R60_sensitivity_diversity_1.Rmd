---
title: "Sensitivity analysis"
output: html_document
date: "2026-05-02"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r}
library(tidyverse)
library(brms)   
library(tidybayes)
library(ggtext)
library(broom.mixed) 
library(cowplot)
```

```{r}
meta <- read_csv('../data/R59_meta_expanded.csv') |> 
  mutate(intensity = factor(intensity, levels = c('nonablative','reduced','ablative'))) %>%
  mutate(pid = factor(pid)) %>%
    # This divides all the new intake columns by 100.
  mutate(across(starts_with("fg_"), ~ .x / 100))

key <- read_csv('../data/food_group_color_key_final.csv', col_types = 'ccccc')
```

# removing EN/TPN

```{r}
# First, identify all 'pid's that have at least one TRUE
# in either the 'EN' or 'TPn' column.
pids_to_remove <- meta %>%
  group_by(pid) %>%
  # Keep only the groups (pids) where *any* row has EN or TPn as TRUE
  filter(any(EN | TPN)) %>%
  ungroup() %>%
  # Get the list of unique pids
  pull(pid) %>%
  unique()

# --- Tallying the removals ---

# 1. Tally up how many unique pids are being removed
num_pids_removed <- length(pids_to_remove)

# 2. Tally up how many total sampleids are linked to those pids
total_samples_removed <- meta %>%
  filter(pid %in% pids_to_remove) %>%
  nrow()

# Print the tallies to the console
print(paste("Number of unique pids to remove:", num_pids_removed))
print(paste("Total sampleids to remove:", total_samples_removed))

# --- Creating the new, filtered dataframe ---

# Finally, filter the original dataframe to *exclude* all rows
# associated with the pids identified for removal.
meta_updated <- meta %>%
  filter(!pid %in% pids_to_remove)
```

```{r}
all_food_vars <- meta_updated %>% select(starts_with('fg')) %>% colnames()

# Create the interaction terms for all food variables in this iteration
interaction_terms <- paste(all_food_vars, "empirical", sep = "*")

# Build the full formula string dynamically
formula_string <- paste(
  "log(simpson_reciprocal) ~ 0 + intensity + empirical + ",
  paste(interaction_terms, collapse = " + "),
  "+ (1 | pid) + (1 | timebin)"
)

# Convert the string to a formula object
formula <- brms::bf(as.formula(formula_string))

# Build the priors by adding them together.
# This single prior() call applies to all coefficients listed in `all_food_coefs`.
priors <-
    prior(normal(0, 1), class = 'b') + # General prior for all food effects
    # Specific priors that override the general one for non-food covariates
    prior(normal(0, 0.5), class = 'b', coef = "empiricalTRUE") +
    prior(normal(2, .1), class = 'b', coef = "intensityablative") +
    prior(normal(2, .1), class = 'b', coef = "intensityreduced") +
    prior(normal(2, .1), class = 'b', coef = "intensitynonablative")

#Fit the model (using fewer iterations for this example to run quickly)
model_fit <- brm(
  formula = formula,
  data = meta_updated,
  prior = priors,
    warmup = 1000, iter = 3000,
    chains = 4, cores = 4,
    seed = 123,
    silent = 2,
  control = list(adapt_delta = 0.99)
)

results_df <- tidy(model_fit, conf.int = TRUE) %>%
  mutate(conf.low = round(conf.low, 2),
         conf.high = round(conf.high, 2)) |> 
  mutate(
    n_samples = nrow(meta_updated),
    n_patients = n_distinct(meta_updated$pid),
  )

results_df |> write_csv('../data/R56_results_df_rm_EN_TPN.csv')
```

## forest remove EN/TPN 

```{r}
# the above figure but make it the way the other figure look like
key <- read_csv('../data/food_group_color_key_final.csv', col_types = 'ccccc')

replacement_dictionary <- setNames(key$shortname, key$fg1_name)

level_order <- rev(c(
    "abx",  
    "abx * Sweets", "Sweets",
    "abx * Grains", "Grains",
    "abx * Milk", "Milk",
    "abx * Eggs", "Eggs",
    "abx * Legumes", "Legumes",
    "abx * Meats", "Meats",
    "abx * Fruits", "Fruits",
    "abx * Oils", "Oils",
    "abx * Vegetables", "Vegetables"
))

cleaned_effects <- results_df %>%
  # Keep only the fixed effects
  filter(effect == "fixed") %>%
  # Create a new column to distinguish main effects from interactions
  mutate(
    effect_type = if_else(str_detect(term, ":"), "Interaction", "Main Effect"),
    # Create clean labels for plotting
    clean_term = term %>%
      str_replace("empiricalTRUE$", "abx") %>%
      str_remove_all("empiricalFALSE:|avg_intake_|TRUE$") %>%
      str_replace_all( replacement_dictionary) %>%
      str_replace("empiricalTRUE:", "abx * ") %>%
      str_replace_all("_", " ")
  ) %>%
  filter(!str_detect(clean_term, 'intensity')) %>%
  # Create a new column to identify significant results
  # The condition is TRUE if conf.low and conf.high have the same sign (i.e., don't cross zero)
  mutate(is_significant = (conf.low * conf.high) > 0) %>%
  mutate(clean_term = factor(clean_term, levels = level_order))

shading_df <- cleaned_effects %>%
  mutate(y_numeric = as.numeric(clean_term)) %>%
  filter(str_detect(clean_term, "\\*")) # Filter for interaction terms

plot_forest <- ggplot(cleaned_effects, aes(x = estimate, y = clean_term)) +
  geom_rect(
    data = shading_df,
    aes(ymin = y_numeric - 0.5, ymax = y_numeric + 0.5, xmin = -Inf, xmax = Inf),
    fill = "#FBEADC", # A light orange/peach color like the example
    alpha = 0.7,
    inherit.aes = FALSE
   ) +
   # Add a vertical line at zero, which represents "no effect"
  geom_vline(xintercept = 0, linetype = "solid", color = "blue",size = 0.5) +
   # Use geom_pointrange to show the estimate (point) and confidence interval (line)
   geom_pointrange(
   aes(xmin = conf.low, xmax = conf.high, color = is_significant),
     size = 0.1,linewidth = .7,
  ) +
  scale_color_manual(
    values = c(
      "TRUE" = "red",
      "FALSE" = "black"
    ), name = "Effect Status" # Legend title
  ) +
  scale_y_discrete( labels = function(x) { ifelse(str_detect(x, '\\*'),  str_glue("<b style='color:royalblue'>{x}</b>"), as.character(x)) }) +
  # Add labels and a clean theme
  labs(
    x = "ln(diversity) change",
    subtitle = "excluding patients\nexposed to EN/TPN",
    y = ''
  ) +
  theme_classic(base_size = 10) +
  theme(legend.position = "none", axis.text.y = element_markdown(), aspect.ratio = 2.5)

plot_forest 
```

# add disease

```{r}
meta_disease <- meta %>% 
  filter(!is.na(disease_lineage)) |> 
  mutate(disease_lineage = factor(disease_lineage))


all_food_vars <- meta_disease %>% select(starts_with('fg')) %>% colnames()

# Create the interaction terms for all food variables in this iteration
interaction_terms <- paste(all_food_vars, "empirical", sep = "*")

# Build the full formula string dynamically
formula_string <- paste(
  "log(simpson_reciprocal) ~ 0 + intensity + empirical + TPN + EN + disease_lineage +",
  paste(interaction_terms, collapse = " + "),
  "+ (1 | pid) + (1 | timebin)"
)

# Convert the string to a formula object
formula <- brms::bf(as.formula(formula_string))

# Build the priors by adding them together.
# This single prior() call applies to all coefficients listed in `all_food_coefs`.
priors <-
    prior(normal(0, 1), class = 'b') + # General prior for all food effects
    # Specific priors that override the general one for non-food covariates
    prior(normal(0, 0.1), class = 'b', coef = "TPNTRUE") +
    prior(normal(0, 0.1), class = 'b', coef = "ENTRUE") +
    prior(normal(0, 0.5), class = 'b', coef = "empiricalTRUE") +
    prior(normal(2, .1), class = 'b', coef = "intensityablative") +
    prior(normal(2, .1), class = 'b', coef = "intensityreduced") +
    prior(normal(2, .1), class = 'b', coef = "intensitynonablative")

#Fit the model (using fewer iterations for this example to run quickly)
model_fit <- brm(
  formula = formula,
  data = meta_disease,
  prior = priors,
    warmup = 1000, iter = 3000,
    chains = 4, cores = 4,
    seed = 123,
    silent = 2,
  control = list(adapt_delta = 0.99)
)
 
results_df_disease <- tidy(model_fit, conf.int = TRUE) %>%
  mutate(conf.low = round(conf.low, 2),
         conf.high = round(conf.high, 2))  |> 
  mutate(
    n_samples = nrow(meta_disease),
    n_patients = n_distinct(meta_disease$pid),
  )

results_df_disease |> write_csv('../data/R60_results_df_adjusting_for_disease_lineage.csv')
```

## forest disease 

```{r}
# the above figure but make it the way the other figure look like
key <- read_csv('../data/food_group_color_key_final.csv', col_types = 'ccccc')

replacement_dictionary <- setNames(key$shortname, key$fg1_name)

level_order <- rev(c(
    "myeloid", "abx", "EN", "TPN",
    "abx * Sweets", "Sweets",
    "abx * Grains", "Grains",
    "abx * Milk", "Milk",
    "abx * Eggs", "Eggs",
    "abx * Legumes", "Legumes",
    "abx * Meats", "Meats",
    "abx * Fruits", "Fruits",
    "abx * Oils", "Oils",
    "abx * Vegetables", "Vegetables"
))

cleaned_effects <- results_df_disease %>%
  # Keep only the fixed effects
  filter(effect == "fixed") %>%
  # Create a new column to distinguish main effects from interactions
  mutate(
    effect_type = if_else(str_detect(term, ":"), "Interaction", "Main Effect"),
    # Create clean labels for plotting
    clean_term = term %>%
      str_replace("empiricalTRUE$", "abx") %>%
      str_remove_all("empiricalFALSE:|avg_intake_|TRUE$") %>%
      str_replace_all( replacement_dictionary) %>%
      str_replace("empiricalTRUE:", "abx * ") %>%
      str_replace_all("_", " ") %>% 
      str_replace("disease lineageMyeloid", "myeloid")
  ) %>%
  filter(!str_detect(clean_term, 'intensity')) %>%
  # Create a new column to identify significant results
  # The condition is TRUE if conf.low and conf.high have the same sign (i.e., don't cross zero)
  mutate(is_significant = (conf.low * conf.high) >= 0) %>%
  mutate(clean_term = factor(clean_term, levels = level_order))

shading_df <- cleaned_effects %>%
  mutate(y_numeric = as.numeric(clean_term)) %>%
  filter(str_detect(clean_term, "\\*")) # Filter for interaction terms

plot_disease <- ggplot(cleaned_effects, aes(x = estimate, y = clean_term)) +
  geom_rect(
    data = shading_df,
    aes(ymin = y_numeric - 0.5, ymax = y_numeric + 0.5, xmin = -Inf, xmax = Inf),
    fill = "#FBEADC", # A light orange/peach color like the example
    alpha = 0.7,
    inherit.aes = FALSE
   ) +
   # Add a vertical line at zero, which represents "no effect"
  geom_vline(xintercept = 0, linetype = "solid", color = "blue",size = 0.5) +
   # Use geom_pointrange to show the estimate (point) and confidence interval (line)
   geom_pointrange(
   aes(xmin = conf.low, xmax = conf.high, color = is_significant),
     size = 0.1,linewidth = .7,
  ) +
  scale_color_manual(
    values = c(
      "TRUE" = "red",
      "FALSE" = "black"
    ), name = "Effect Status" # Legend title
  ) +
  scale_y_discrete( labels = function(x) { ifelse(str_detect(x, '\\*'),  str_glue("<b style='color:royalblue'>{x}</b>"), as.character(x)) }) +
  # Add labels and a clean theme
  labs(
    x = "ln(diversity) change",
    subtitle = "model including\na term for underlying\nmalignant disease",
    y = ''
  ) +
  theme_classic(base_size = 10) +
  theme(legend.position = "none", axis.text.y = element_markdown(), aspect.ratio = 5.5)

plot_disease
```

# add the HCT-CI

```{r}
meta_CI <- meta %>%
  filter(!is.na(ci_cleaned_numeric))
```

```{r}
all_food_vars <- meta_CI %>% select(starts_with('fg')) %>% colnames()

# Create the interaction terms for all food variables in this iteration
interaction_terms <- paste(all_food_vars, "empirical", sep = "*")

# Build the full formula string dynamically
formula_string <- paste(
  "log(simpson_reciprocal) ~ 0 + intensity + empirical + TPN + EN + ci_cleaned_numeric +",
  paste(interaction_terms, collapse = " + "),
  "+ (1 | pid) + (1 | timebin)"
)

# Convert the string to a formula object
formula <- brms::bf(as.formula(formula_string))

# Build the priors by adding them together.
# This single prior() call applies to all coefficients listed in `all_food_coefs`.
priors <-
    prior(normal(0, 1), class = 'b') + # General prior for all food effects
    # Specific priors that override the general one for non-food covariates
    prior(normal(0, 0.1), class = 'b', coef = "TPNTRUE") +
    prior(normal(0, 0.1), class = 'b', coef = "ENTRUE") +
    prior(normal(0, 0.5), class = 'b', coef = "empiricalTRUE") +
    prior(normal(2, .1), class = 'b', coef = "intensityablative") +
    prior(normal(2, .1), class = 'b', coef = "intensityreduced") +
    prior(normal(2, .1), class = 'b', coef = "intensitynonablative")

#Fit the model (using fewer iterations for this example to run quickly)
model_fit <- brm(
  formula = formula,
  data = meta_CI,
  prior = priors,
    warmup = 1000, iter = 3000,
    chains = 4, cores = 4,
    seed = 123,
    silent = 2,
  control = list(adapt_delta = 0.99)
)

results_df <- tidy(model_fit, conf.int = TRUE) %>%
  mutate(conf.low = round(conf.low, 2),
         conf.high = round(conf.high, 2)) |> 
  mutate(
    n_samples = nrow(meta_CI),
    n_patients = n_distinct(meta_CI$pid),
  )

results_df |> write_csv('../data/R60_results_df_adjusting_for_CI.csv')
```

## forest CI 

```{r}
# the above figure but make it the way the other figure look like
key <- read_csv('../data/food_group_color_key_final.csv', col_types = 'ccccc')

replacement_dictionary <- setNames(key$shortname, key$fg1_name)

level_order <- rev(c(
    "HCT-CI", "abx", "EN", "TPN", 
    "abx * Sweets", "Sweets",
    "abx * Grains", "Grains",
    "abx * Milk", "Milk",
    "abx * Eggs", "Eggs",
    "abx * Legumes", "Legumes",
    "abx * Meats", "Meats",
    "abx * Fruits", "Fruits",
    "abx * Oils", "Oils",
    "abx * Vegetables", "Vegetables"
))

cleaned_effects <- results_df %>%
  # Keep only the fixed effects
  filter(effect == "fixed") %>%
  # Create a new column to distinguish main effects from interactions
  mutate(
    effect_type = if_else(str_detect(term, ":"), "Interaction", "Main Effect"),
    # Create clean labels for plotting
    clean_term = term %>%
      str_replace("empiricalTRUE$", "abx") %>%
      str_remove_all("empiricalFALSE:|avg_intake_|TRUE$") %>%
      str_replace_all( replacement_dictionary) %>%
      str_replace("empiricalTRUE:", "abx * ") %>%
      str_replace_all("_", " ") %>% 
      str_replace("ci cleaned numeric", "HCT-CI")
  ) %>%
  filter(!str_detect(clean_term, 'intensity')) %>%
  # Create a new column to identify significant results
  # The condition is TRUE if conf.low and conf.high have the same sign (i.e., don't cross zero)
  mutate(is_significant = (conf.low * conf.high) >= 0) %>%
  mutate(clean_term = factor(clean_term, levels = level_order))

shading_df <- cleaned_effects %>%
  mutate(y_numeric = as.numeric(clean_term)) %>%
  filter(str_detect(clean_term, "\\*")) # Filter for interaction terms

plot_CI <- ggplot(cleaned_effects, aes(x = estimate, y = clean_term)) +
  geom_rect(
    data = shading_df,
    aes(ymin = y_numeric - 0.5, ymax = y_numeric + 0.5, xmin = -Inf, xmax = Inf),
    fill = "#FBEADC", # A light orange/peach color like the example
    alpha = 0.7,
    inherit.aes = FALSE
   ) +
   # Add a vertical line at zero, which represents "no effect"
  geom_vline(xintercept = 0, linetype = "solid", color = "blue",size = 0.5) +
   # Use geom_pointrange to show the estimate (point) and confidence interval (line)
   geom_pointrange(
   aes(xmin = conf.low, xmax = conf.high, color = is_significant),
     size = 0.1,linewidth = .7,
  ) +
  scale_color_manual(
    values = c(
      "TRUE" = "red",
      "FALSE" = "black"
    ), name = "Effect Status" # Legend title
  ) +
  scale_y_discrete( labels = function(x) { ifelse(str_detect(x, '\\*'),  str_glue("<b style='color:royalblue'>{x}</b>"), as.character(x)) }) +
  # Add labels and a clean theme
  labs(
    x = "ln(diversity) change",
    subtitle = "model including\na term for\ncomorbidities",
    y = ''
  ) +
  theme_classic(base_size = 10) +
  theme(legend.position = "none", axis.text.y = element_markdown(), aspect.ratio = 5.5)

plot_CI 
```

# add the PCA

```{r}
meta_pca <- meta |>
  filter(!is.na(PCA))

all_food_vars <- meta_pca |> select(starts_with('fg')) |> colnames()

# Set up interactions 
interaction_terms <- paste(all_food_vars, "empirical", sep = "*")

# The formula includes the PCA term to adjust for broader compositional shifts
# and EN/TPN to control for alternative feeding methods
formula_string <- paste(
  "log(simpson_reciprocal) ~ 0 + intensity + empirical + PCA + EN + TPN +",
  paste(interaction_terms, collapse = " + "),
  "+ (1 | pid) + (1 | timebin)"
)

formula <- brms::bf(as.formula(formula_string))

priors <-
    prior(normal(0, 1), class = 'b') + 
    prior(normal(0, 0.1), class = 'b', coef = "TPNTRUE") +
    prior(normal(0, 0.1), class = 'b', coef = "ENTRUE") +
    prior(normal(0, 0.5), class = 'b', coef = "empiricalTRUE") +
    prior(normal(2, .1), class = 'b', coef = "intensityablative") +
    prior(normal(2, .1), class = 'b', coef = "intensityreduced") +
    prior(normal(2, .1), class = 'b', coef = "intensitynonablative")

model_fit_pca <- brm(
  formula = formula,
  data = meta_pca,
  prior = priors,
  warmup = 1000, iter = 3000,
  chains = 4, cores = 4,
  seed = 123,
  silent = 2,
  control = list(adapt_delta = 0.99)
)

results_df_pca <- tidy(model_fit_pca, conf.int = TRUE) |>
  mutate(conf.low = round(conf.low, 2),
         conf.high = round(conf.high, 2)) |> 
  mutate(
    n_samples = nrow(meta_pca),
    n_patients = n_distinct(meta_pca$pid)
  )

results_df_pca |> write_csv('../data/R60_results_df_adjusting_for_PCA.csv')
```

## forest PCA


```{r}
replacement_dictionary <- setNames(key$shortname, key$fg1_name)

level_order <- rev(c(
    "PCA", "abx", "EN", "TPN", 
    "abx * Sweets", "Sweets",
    "abx * Grains", "Grains",
    "abx * Milk", "Milk",
    "abx * Eggs", "Eggs",
    "abx * Legumes", "Legumes",
    "abx * Meats", "Meats",
    "abx * Fruits", "Fruits",
    "abx * Oils", "Oils",
    "abx * Vegetables", "Vegetables"
))

cleaned_effects <- results_df_pca |>
  filter(effect == "fixed") |>
  mutate(
    effect_type = if_else(str_detect(term, ":"), "Interaction", "Main Effect"),
    clean_term = term |>
      str_replace("empiricalTRUE$", "abx") |>
      str_remove_all("empiricalFALSE:|avg_intake_|TRUE$") |>
      str_replace_all(replacement_dictionary) |>
      str_replace("empiricalTRUE:", "abx * ") |>
      str_replace_all("_", " ") 
  ) |>
  filter(!str_detect(clean_term, 'intensity')) |>
  mutate(is_significant = (conf.low * conf.high) >= 0) |>
  mutate(clean_term = factor(clean_term, levels = level_order))

shading_df <- cleaned_effects |>
  mutate(y_numeric = as.numeric(clean_term)) |>
  filter(str_detect(clean_term, "\\*")) 

plot_pca <- ggplot(cleaned_effects, aes(x = estimate, y = clean_term)) +
  geom_rect(
    data = shading_df,
    aes(ymin = y_numeric - 0.5, ymax = y_numeric + 0.5, xmin = -Inf, xmax = Inf),
    fill = "#FBEADC", 
    alpha = 0.7,
    inherit.aes = FALSE
   ) +
  geom_vline(xintercept = 0, linetype = "solid", color = "blue", size = 0.8) +
  geom_pointrange(
   aes(xmin = conf.low, xmax = conf.high, color = is_significant),
     size = 0.25, linewidth = 1,
  ) +
  scale_color_manual(
    values = c("TRUE" = "red", "FALSE" = "black"), 
    name = "Effect Status" 
  ) +
  scale_y_discrete(labels = function(x) { ifelse(str_detect(x, '\\*'),  str_glue("<b style='color:royalblue'>{x}</b>"), as.character(x)) }) +
  labs(
    x = "ln(diversity) change",
    subtitle = "model including\na term for\npatient-\ncontrolled analgesia",
    y = ''
  ) +
  theme_classic(base_size = 11) +
  theme(legend.position = "none", axis.text.y = element_markdown())

plot_pca
```

# empiric only exposed stool samples

```{r}
meta_empiric <- meta |> 
  filter(exposure_type %in% c('empiric','no_broad_spectrum_exposure')) |> 
  mutate(exposure_type = factor(exposure_type, levels = c('no_broad_spectrum_exposure','empiric'))) 

all_food_vars <- meta_empiric |> select(starts_with('fg')) |> colnames()

interaction_terms <- paste(all_food_vars, "exposure_type", sep = "*")

# Build the full formula string dynamically
# Replaced 'empirical' with 'exposure_type'
formula_string <- paste(
  "log(simpson_reciprocal) ~ 0 + intensity + exposure_type + TPN + EN +",
  paste(interaction_terms, collapse = " + "),
  "+ (1 | pid) + (1 | timebin)"
)

# Convert the string to a formula object
formula <- brms::bf(as.formula(formula_string))

# Build the priors.
# Note the change in the coefficient name for the antibiotic effect from
# "empiricalTRUE" to "exposure_typeempiric". brms creates this name by
# combining the variable name ('exposure_type') and the non-reference level ('empiric').
priors <-
    prior(normal(0, 1), class = 'b') + # General prior for all food effects
    # Specific priors that override the general one for non-food covariates
    prior(normal(0, 0.1), class = 'b', coef = "TPNTRUE") +
    prior(normal(0, 0.1), class = 'b', coef = "ENTRUE") +
    prior(normal(0, 0.5), class = 'b', coef = "exposure_typeempiric") +
    prior(normal(2, .1), class = 'b', coef = "intensityablative") +
    prior(normal(2, .1), class = 'b', coef = "intensityreduced") +
    prior(normal(2, .1), class = 'b', coef = "intensitynonablative")

model_fit_empiric <- brm(
  formula = formula,
  data = meta_empiric,
  prior = priors,
  warmup = 1000, iter = 3000,
  chains = 4, cores = 4,
  seed = 123,
  silent = 2,
  control = list(adapt_delta = 0.99)
)

results_df_empiric <- tidy(model_fit_empiric, conf.int = TRUE) |>
  mutate(conf.low = round(conf.low, 2),
         conf.high = round(conf.high, 2)) |> 
  mutate(
    n_samples = nrow(meta_empiric),
    n_patients = n_distinct(meta_empiric$pid)
  )

results_df_empiric |> write_csv('../data/R60_results_df_empiric_only.csv')
```

## forest empiric only

```{r}
replacement_dictionary <- setNames(key$shortname, key$fg1_name)

level_order <- rev(c(
    "empiric only", "EN", "TPN",
    "empiric only * Sweets", "Sweets",
    "empiric only * Grains", "Grains",
    "empiric only * Milk", "Milk",
    "empiric only * Eggs", "Eggs",
    "empiric only * Legumes", "Legumes",
    "empiric only * Meats", "Meats",
    "empiric only * Fruits", "Fruits",
    "empiric only * Oils", "Oils",
    "empiric only * Vegetables", "Vegetables"
))

cleaned_effects <- results_df_empiric %>%
  # Keep only the fixed effects
  filter(effect == "fixed") %>%
  # Create a new column to distinguish main effects from interactions
  mutate(
    effect_type = if_else(str_detect(term, ":"), "Interaction", "Main Effect"),
    # Create clean labels for plotting
    clean_term = term %>%
      str_replace("exposure_typeempiric$", "empiric_only") %>%
      str_remove_all("exposure_typeno_broad_spectrum_exposure:|avg_intake_|TRUE$") %>%
      str_replace_all( replacement_dictionary) %>% 
      str_replace("exposure_typeempiric:", "empiric_only * ") %>%
      str_replace_all("_", " ")
  ) %>% 
  filter(!str_detect(clean_term, 'intensity')) %>%
  # Create a new column to identify significant results
  # The condition is TRUE if conf.low and conf.high have the same sign (i.e., don't cross zero)
  mutate(is_significant = (conf.low * conf.high) >= 0) %>% 
  mutate(clean_term = factor(clean_term, levels = level_order))

shading_df <- cleaned_effects %>%
  mutate(y_numeric = as.numeric(clean_term)) %>%
  filter(str_detect(clean_term, "\\*")) # Filter for interaction terms

plot_stratify <- ggplot(cleaned_effects, aes(x = estimate, y = clean_term)) +
  geom_rect(
    data = shading_df,
    aes(ymin = y_numeric - 0.5, ymax = y_numeric + 0.5, xmin = -Inf, xmax = Inf),
    fill = "#FBEADC", # A light orange/peach color like the example
    alpha = 0.7,
    inherit.aes = FALSE
   ) +
   # Add a vertical line at zero, which represents "no effect"
  geom_vline(xintercept = 0, linetype = "solid", color = "blue",size = 0.8) +
   # Use geom_pointrange to show the estimate (point) and confidence interval (line)
   geom_pointrange(
   aes(xmin = conf.low, xmax = conf.high, color = is_significant),
     size = 0.25,linewidth = 1,
  ) +
  scale_color_manual(
    values = c(
      "TRUE" = "red",
      "FALSE" = "black"
    ), name = "Effect Status" # Legend title
  ) +
  scale_y_discrete( labels = function(x) { ifelse(str_detect(x, '\\*'),  str_glue("<b style='color:royalblue'>{x}</b>"), as.character(x)) }) +
  # Add labels and a clean theme
  labs(
    x = "ln(diversity) change",
    title = "excluding samples\ncollected during\ntreatment of infection",
    y = ''
  ) +
  theme_classic(base_size = 11) +
  theme(legend.position = "none", axis.text.y = element_markdown())
  
plot_stratify  
```

# only pre-transplant samples

```{r}
# Subsetting to study days relative to transplant strictly before day 0
meta_pre <- meta |>
  filter(sdrt < 0)

all_food_vars <- meta_pre |> select(starts_with('fg')) |> colnames()

interaction_terms <- paste(all_food_vars, "empirical", sep = "*")

formula_string <- paste(
  "log(simpson_reciprocal) ~ 0 + intensity + empirical + EN + TPN +",
  paste(interaction_terms, collapse = " + "),
  "+ (1 | pid) + (1 | timebin)"
)

formula <- brms::bf(as.formula(formula_string))

priors <-
    prior(normal(0, 1), class = 'b') + 
    prior(normal(0, 0.1), class = 'b', coef = "TPNTRUE") +
    prior(normal(0, 0.1), class = 'b', coef = "ENTRUE") +
    prior(normal(0, 0.5), class = 'b', coef = "empiricalTRUE") +
    prior(normal(2, .1), class = 'b', coef = "intensityablative") +
    prior(normal(2, .1), class = 'b', coef = "intensityreduced") +
    prior(normal(2, .1), class = 'b', coef = "intensitynonablative")

model_fit_pre <- brm(
  formula = formula,
  data = meta_pre,
  prior = priors,
  warmup = 1000, iter = 3000,
  chains = 4, cores = 4,
  seed = 123,
  silent = 2,
  control = list(adapt_delta = 0.99)
)

results_df_pre <- tidy(model_fit_pre, conf.int = TRUE) |>
  mutate(conf.low = round(conf.low, 2),
         conf.high = round(conf.high, 2)) |> 
  mutate(
    n_samples = nrow(meta_pre),
    n_patients = n_distinct(meta_pre$pid)
  )

results_df_pre |> write_csv('../data/R60_results_df_pre_transplant.csv')  
```

## forest pre-transplant

```{r}
level_order <- rev(c(
    "abx",  "EN", "TPN", 
    "abx * Sweets", "Sweets",
    "abx * Grains", "Grains",
    "abx * Milk", "Milk",
    "abx * Eggs", "Eggs",
    "abx * Legumes", "Legumes",
    "abx * Meats", "Meats",
    "abx * Fruits", "Fruits",
    "abx * Oils", "Oils",
    "abx * Vegetables", "Vegetables"
))

cleaned_effects <- results_df_pre |>
  filter(effect == "fixed") |>
  mutate(
    effect_type = if_else(str_detect(term, ":"), "Interaction", "Main Effect"),
    clean_term = term |>
      str_replace("empiricalTRUE$", "abx") |>
      str_remove_all("empiricalFALSE:|avg_intake_|TRUE$") |>
      str_replace_all(replacement_dictionary) |>
      str_replace("empiricalTRUE:", "abx * ") |>
      str_replace_all("_", " ")
  ) |>
  filter(!str_detect(clean_term, 'intensity')) |>
  mutate(is_significant = (conf.low * conf.high) >= 0) |>
  mutate(clean_term = factor(clean_term, levels = level_order))

shading_df <- cleaned_effects |>
  mutate(y_numeric = as.numeric(clean_term)) |>
  filter(str_detect(clean_term, "\\*")) 

plot_pre <- ggplot(cleaned_effects, aes(x = estimate, y = clean_term)) +
  geom_rect(
    data = shading_df,
    aes(ymin = y_numeric - 0.5, ymax = y_numeric + 0.5, xmin = -Inf, xmax = Inf),
    fill = "#FBEADC", 
    alpha = 0.7,
    inherit.aes = FALSE
   ) +
  geom_vline(xintercept = 0, linetype = "solid", color = "blue", size = 0.8) +
  geom_pointrange(
   aes(xmin = conf.low, xmax = conf.high, color = is_significant),
     size = 0.25, linewidth = 1,
  ) +
  scale_color_manual(
    values = c("TRUE" = "red", "FALSE" = "black"), 
    name = "Effect Status" 
  ) +
  scale_y_discrete(labels = function(x) { ifelse(str_detect(x, '\\*'),  str_glue("<b style='color:royalblue'>{x}</b>"), as.character(x)) }) +
  labs(
    x = "ln(diversity) change",
    subtitle = "restricting to only pre-\ntransplant samples",
    y = ''
  ) +
  theme_classic(base_size = 11) +
  theme(legend.position = "none", axis.text.y = element_markdown())

plot_pre
 
```
# assemble 

```{r}
final_plot <- plot_grid( 
 plot_forest, 
 plot_disease ,
 plot_CI,
 plot_pca,
 plot_stratify,
 plot_pre,
 ncol = 3)

title <- ggdraw() + 
  draw_label("Extended Fig. E5",fontface = 'bold',x = 0, hjust = 0) +
  theme(plot.margin = margin(0, 0, 0, 0))

combined <- plot_grid(
  title, final_plot,
  ncol = 1,
  # rel_heights values control vertical title margins
  rel_heights = c(0.1, 1)
) +theme(plot.margin = unit(c(0,0,0,0), "cm")) 

ggsave('../data/E05_R60_sensitivity_diversity_1.pdf',
      width = 8.5, height = 11, units = "in", device = 'pdf', plot = combined,
      dpi = 300)  
```

```{r}
ggsave(
  '../data/E05f_middle_R60.pdf', 
  plot = plot_CI, 
  width = 100, 
  height = 100, 
  units = "mm", 
  device = 'pdf'
)

ggsave(
  '../data/E05f_left_R60.pdf', 
  plot = plot_disease, 
  width = 100, 
  height = 100, 
  units = "mm", 
  device = 'pdf'
)

ggsave(
  '../data/E05e_R60.pdf', 
  plot = plot_forest, 
  width = 100, 
  height = 100, 
  units = "mm", 
  device = 'pdf'
)
```

