...
data_clean_patients |> distinct(pid)
```
...
results_df <- tidy(model_fit, conf.int = TRUE) %>%
  ...
  mutate(
    n_samples = nrow(data_no_outliers),
    n_patients = n_distinct(data_no_outliers$pid),
  )

results_df |> write_csv('../data/R56_results_df_rm_super_sugar_eaters.csv')
```
