...
genus_colors_all <- asv_annotation_blast_color_ag |> distinct(genus, color)
```
...
asv_relab_97 <- read_csv('../data/171_quality_asv_relab_pident97_genus.csv') |> 
  inner_join(asv_annotation_blast_color_ag |> distinct(asv_key, color, color_label_group_distinct)) |> 
...
plot_data <- pcoa_scores_with_ids %>%
  ...
  full_join(meta |> select(sampleid, simpson_reciprocal))
...
special_statuses <- c("Enterococcus_faecium",  "Diverse/None", "Other")
...
contrasting_colors <- genus_colors_all |> 
  filter(genus %in% genera_to_color) |> deframe()
...
final_color_vector <- c(
  ...
  "Enterococcus_faecium" = "#129246",    # Dark Green
  "Diverse/None"         = "#333333",    
  "Other"                = "#AFB4B5"     # Medium Gray
)
...
pcoa_plot <- ggplot(
  ...
  theme_bw() +
  theme(
    plot.margin = unit(c(1, 1, 1, 1), "cm"), # Give extra space on top/right/left
...
# Display the plot
# print(pcoa_plot)
ggsave('../data/R30_pcoa_domination.pdf', width = 4)                                                 
```

### asv level pcoa 

```{r}
# 1. PREPARE THE MATH MATRIX
# We need a matrix where rows are samples and columns are ASVs
# Using count_relative as the abundance metric
asv_matrix <- asv_relab_97 |> 
  select(sampleid, asv_key, count_relative) |> 
  # pivot to wide format: one row per sample, one column per ASV
  pivot_wider(names_from = asv_key, values_from = count_relative, values_fill = 0) |> 
  column_to_rownames("sampleid") |> 
  as.matrix()

# 2. CALCULATE DISTANCE & PCoA
# calculating bray-curtis distance on the raw ASV data
dist_matrix <- vegdist(asv_matrix, method = "bray")

# perform the PCoA (MDS)
pcoa_results <- cmdscale(dist_matrix, k = 2, eig = TRUE)

# 3. PREPARE PLOT DATA
# extract coordinates and calculate percent variance explained
axis_1_var <- round(pcoa_results$eig[1] / sum(pcoa_results$eig) * 100, 2)
axis_2_var <- round(pcoa_results$eig[2] / sum(pcoa_results$eig) * 100, 2)
```


```{r}
pcoa_df <- pcoa_results$points |> 
  as.data.frame() |> 
  rownames_to_column("sampleid") |> 
  rename(Axis1 = V1, Axis2 = V2) |> 
  # join back the metadata logic
  # finding the top bug per sample to determine the color
  left_join(
    asv_relab_97 |> 
      group_by(sampleid) |>
      # grab the single row with the max abundance for this sample
      slice_max(count_relative, n = 1, with_ties = FALSE) |>
      select(sampleid, color, color_label_group_distinct),
    by = "sampleid"
  ) |> 
  left_join(asv_relab_97 |> distinct(sampleid, simpson_reciprocal))

# 4. VISUALIZE
# plotting using the pre-assigned hex codes in your 'color' column
pcoa_asv_most <- pcoa_df |> 
  ggplot(aes(x = Axis1, y = Axis2)) +
  geom_point(aes(color = color), size = scatter_size, alpha = 0.7, shape = 16) +
    # Add the dashed zero-lines
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  # this function tells ggplot to use the actual hex codes in the column
  scale_color_identity(guide = "legend", 
                       labels = pcoa_df$color_label_group_distinct, 
                       breaks = pcoa_df$color) +
  labs(
    x = paste0("PCoA 1 (", axis_1_var, "%)"),
    y = paste0("PCoA 2 (", axis_2_var, "%)")
  ) +
  theme_bw() +
...
ggsave('../data/R30_pcoa_asv_most_abundant.pdf', width = 7)    
```
```{r}
# same asv level pcoa but colored with diversity  
pcoa_asv_diversity <- ggplot(
  pcoa_df,
  aes(x = Axis1, y = Axis2, color = simpson_reciprocal)
) +
  # Add the dashed zero-lines
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  ...
  scale_color_gradientn( name = bquote(""~alpha*"-Diversity (inverse Simpson index)"),
                         values = c(0, 0.4,  1), colours = c("#1E90FF", "#FFFF00", "#FF3030"), 
                         trans="log",
...
ggsave('../data/R30_pcoa_with_diversity_asv.pdf', width = 7, height = 5)

top_row_asv <- plot_grid( pcoa_asv_most , 
                      pcoa_asv_diversity,
                nrow  = 1, 
                labels = 'auto',
                align = 'vh')
...
ggsave('../data/R30_axis_model_asv.pdf',
      width = 8.5, height = 11, units = "in", device = 'pdf', plot = top_row_asv,
      dpi = 300)   
```



### diversity pcoa with customized color palette

```{r}
# -----------------------------------------------------------------
# --- 11. Create the NEW DIVERSITY PCoA Plot ---
# -----------------------------------------------------------------
# This plot uses the *exact same* PCoA and annotations,
# but re-colors the points by alpha diversity.

pcoa_plot_diversity <- ggplot(
  plot_data,
  # --- KEY CHANGE: Color by simpson_reciprocal ---
  aes(x = Axis.1, y = Axis.2, color = simpson_reciprocal)
) +
  # Add the dashed zero-lines
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  ...
  #   option = "C" # Use the 'viridis' palette
  # ) +
  scale_color_gradientn( name = bquote(""~alpha*"-Diversity (inverse Simpson index)"),
                         values = c(0, 0.4,  1), colours = c("#1E90FF", "#FFFF00", "#FF3030"), 
                         trans="log",
  ...
  # Add labels showing variance explained
  labs(
   # title = "Bray-Curtis PCoA (Custom Genus/ASV) Colored by Alpha Diversity",
    x = sprintf("Axis 1 (%.2f%%)", variance_explained[1]),
    y = sprintf("Axis 2 (%.2f%%)", variance_explained[2])
  ) +
  theme_bw() +
...
# Display the new plot
print(pcoa_plot_diversity)
#ggsave('../data/R30_pcoa_with_diversity.pdf', width = 7, height = 5)
```
...
```{r}
library(rmcorr)
# Combined exposure to sweets and antibiotics was also associated with a shift toward low-diversity compositions characterized by community monodomination, for example by Enterococcus faecium.


rmc_result <- rmcorr(participant = pid, measure1 = Axis.1, measure2 = simpson_reciprocal, dataset = meta_Axis)

# Visualizing the within-subject relationship
meta_Axis |> 
  ggplot(aes(x = Axis.1, y = simpson_reciprocal)) +
  geom_point(alpha = 0.6) + 
  # Connecting points from the same participant highlights the repeated measure aspect
  # This adds the parallel slopes characteristic of rmcorr (visual aid)
  # It shows the trend assuming the slope is shared across participants
  geom_line(stat = "smooth", method = "lm", se = FALSE, alpha = 0.5) +
  labs(
    title = "Repeated Measures Correlation: Axis 1 vs. Simpson's",
    subtitle = paste("rmcorr r:", round(rmc_result$r, 2), 
                     "| P-value:", format.pval(rmc_result$p, digits = 3)),
    x = "Axis 1",
    y = "Simpson Reciprocal Index",
    color = "Patient ID"
  ) +
  theme_minimal()
```
...
```{r}
all_results <- read_csv('../data/R30_all_results_axis.csv')
...
```{r}
top_row <- plot_grid( pcoa_plot , 
                      pcoa_plot_diversity,
                nrow  = 1, 
                labels = 'auto',
                align = 'vh')

final_plot <- plot_grid( top_row, 
                         plot_axis ,
                ncol  = 1, 
                rel_heights = c(2, 2),
                labels = c(NA, 'c'))
...
# Model with asv level pcoa coord
...
meta_Axis_asv <- meta %>% 
  ...
  mutate(pid = factor(pid)) %>%
    # This divides all the new intake columns by 100.
  mutate(across(starts_with("fg_"), ~ .x / 100)) %>% 
  inner_join(pcoa_df |> select(sampleid:color_label_group_distinct)) %>% 
  mutate(timebin = cut_width(sdrt, 7, boundary=0, closed = 'left')) 

# Define the outcomes you want to test
outcomes_to_run <- c("Axis1", "Axis2")

# Get all food variable names from your data (matches screenshot)
all_food_vars <- meta_Axis_asv %>%
  select(starts_with('fg_')) %>%
  colnames()
...
# combining them into a single, convenient data frame.
# `map_dfr` means "map and combine into a dataframe"
all_results_asv <- purrr::map_dfr(model_fits_list, ~ {
...
# --- Display the final combined results ---
print("--- Combined Model Results ---")
print(as.data.frame(all_results_asv))

all_results_asv |> write_csv('../data/R30_all_results_axis_asv.csv')
```
```{r}
all_results_asv <- read_csv('../data/R30_all_results_axis_asv.csv')
...
cleaned_effects <- all_results_asv %>%
  # Keep only the fixed effects
  filter(effect == "fixed") %>%
  # Create a new column to distinguish main effects from interactions
  mutate(
...
plot_axis_asv <- ggplot(cleaned_effects, aes(x = estimate, y = clean_term)) +
  geom_rect(
    data = shading_df,
    aes(ymin = y_numeric - 0.5, ymax = y_numeric + 0.5, xmin = -Inf, xmax = Inf),
    fill = "#FBEADC", # A light orange/peach color like the example
...
final_plot <- plot_grid( top_row_asv, 
                         plot_axis_asv ,
                ncol  = 1, 
                rel_heights = c(2, 2),
                labels = c(NA, 'c'))
