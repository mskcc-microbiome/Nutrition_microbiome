...
source("../functions/save_ggplot_data_to_excel.R")
```
...
newest %>% dplyr::count(Experiment, abx_treatment, diet_treatment)
```
...
experiment_data %>% dplyr::count(strata, treatment)
```
...
longitudinal_plot
save_ggplot_data_to_excel(nofiber_days, plotdf, "../mouse_data.xlsx", "Sup_Figure_8k_delayed_sucrose_CFU", col_names = c(
  "xvar" = "Treatment + Day",
  "logcfu" = "Enterococcal CFU/gram stool",
  "grp" = "Treatment Group"))
...
save_ggplot_data_to_excel(auc_plot, auc_data_viz, "../mouse_data.xlsx", "Sup_Figure_8j_delayed_sucrose_auc", col_names = c(
  "auc" = "AUC of CFU/gram *days",
...
processed_weights_tally <- processed_weights %>% 
  filter(day == 1) %>% 
  dplyr::count(experiment_no, diet_treatment, antibiotic_treatment) 
```
...
analysis_data <- processed_weights %>%
  ...
  mutate(
    weight_change = mice_weights - dplyr::first(mice_weights)
  ) %>%
  ungroup()
...
analysis_data_4_to_day3 <- processed_weights %>%
  ...
  group_by(mouse_identifier) %>%
  mutate(
    weight_gain = mice_weights - dplyr::first(mice_weights)
  ) %>%
  ungroup()
...
weight_data2_ancova <- weight_data2 %>%
  ...
  mutate(baseline_weight = dplyr::first(mice_weights)) %>%
  ungroup() %>%
...
plot_data_percent <- processed_weights %>%
  mutate(mouse_identifier = str_glue('{experiment_no}__{mouse_identifier}__{antibiotic_treatment}__{diet_treatment}'), day = factor(day)) %>% 
  # Make sure data is ordered correctly to find the first day's weight
  arrange( mouse_identifier, day) %>%
  # Group by each unique mouse
  group_by(experiment_no, mouse_identifier, antibiotic_treatment,diet_treatment ) %>%
  # Calculate percent change relative to the first measurement
  mutate(
    baseline_weight = dplyr::first(mice_weights),
    percent_change = ((mice_weights - baseline_weight) / baseline_weight) * 100
  ) %>%
  ungroup() |> 
  relocate(percent_change, .after = day)
...
timeline_percent <- ggplot(plot_data_percent, aes(x = day, y = percent_change)) +
 ...
 # Calculate and plot a line connecting the medians.
  ...
  scale_color_manual(values = color_palette) +
  # 4. Split the plot into two panels based on the diet treatment.
  facet_grid(antibiotic_treatment~diet_treatment) +
  # Add informative labels and a clean theme
  labs(
...
ggsave('../data/R31_mice_weight_timeline_percent_all_treatments.pdf',
      width = 3.5, height = 6, units = "in", device = 'pdf', plot = timeline_percent, 
      dpi = 300)  
```
```{r}
# Are there host health impacts (e.g. weight loss) associated with this bloom
bloom_data_percent <- plot_data_percent |> 
  filter(diet_treatment == 'regular_water', day != 6)

bloom_percent <- ggplot(bloom_data_percent, aes(x = day, y = percent_change)) +
...
faceted_plot <- plot_data_percent %>%
  # Filter to only include Day 3 data
  filter(day == 3) %>%
  # Initialize the ggplot object
  ggplot(aes(
    x = log10_cf_us_per_gram_stool,
    y = percent_change,
    color = antibiotic_treatment
  )) +
  # Add the raw data points
  geom_point(alpha = 0.7, size = 2) +
  # Add linear regression lines (method = "lm").
  # By default, 'se = TRUE' which shows the 95% confidence interval ribbon.
  # This is what you requested.
  #geom_smooth(method = "lm") +
  # Add a dashed horizontal line at 0% weight change for reference
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
  # This is the key command: it splits the plot into separate panels
  # for each level of 'diet_treatment'.
  facet_wrap(~ diet_treatment) +
  # Clean up the labels to be more readable
  labs(
    title = "Health Impact of Enterococcus Bloom by Diet",
    subtitle = "Day 3 Measurements",
    x = "Enterococcus Bloom (Log10 CFU/gram stool)",
    y = "Weight Change from Baseline (%)",
    color = "Antibiotic"
  ) +
  # Use a clean theme
...
# To display the plot, just run the variable name
print(faceted_plot)

ggsave('../data/R31_mice_weight_timeline_percent_faceted_plot.pdf',
      width = 8.5, height = 5, units = "in", device = 'pdf', plot = faceted_plot, 
      dpi = 300)  
```

## model to see the bloom on the host health impacts

```{r}
# the majority of the red dots (the main cluster between ~2.0 and ~3.0 logCFU) look like a random cloud, suggesting no real relationship between the bloom and weight change in the PBS group. The slightly negative slope you see is likely an artifact of that single point.
# 
# Why this is actually good for your hypothesis:
# 
# This makes your main finding stronger, not weaker. Your core question is about the health impact of the bloom.
# 
# PBS Group (Controls): No clear relationship. Bloom level doesn't seem to correlate with weight change (the line is probably flat, with a slope of 0).
# 
# Biapenem Group (Treated): A clear negative relationship. The higher the bloom, the lower the average weight.
# 
# Your story isn't "the slopes are -0.5 and -1.0". Your story is "the Enterococcus bloom had no discernible health impact in healthy control mice, but was strongly associated with weight loss in antibiotic-treated mice."
# 
# The statistical model we discussed (... logCFU * treatment) is designed to test exactly this. The interaction term will essentially check: "Is the slope of the teal line significantly different from the slope of the red line?"
day3_data <- plot_data_percent %>%
  # We only want to plot the measurements from Day 3 (the bloom day)
  filter(day == 3) |> 
  mutate(antibiotic_treatment = factor(antibiotic_treatment, levels = c('biapenem','PBS')))

# --- 3. Fit the Linear Model with Interaction ---
# This model tests if the relationship (the slope) between
# CFU and weight change is DIFFERENT for each treatment group.
#
# The R formula `y ~ x * group` is shorthand for:
# `y ~ x + group + x:group`
#
# The `x:group` term (the interaction) is the key. It directly
# tests if the slopes are different.
interaction_model <- lm(
  percent_change ~ log10_cf_us_per_gram_stool * antibiotic_treatment * diet_treatment,
  data = day3_data
)

# --- 4. View the Model Results ---
# Using tidy() from the 'broom' package gives a clean table
# instead of the standard R summary() output.
model_summary <- tidy(interaction_model)

print("Model Summary Table:")
print(model_summary)

# this above three way interaction tmodel is testing if the relationship (slope) between logCFU and percent_change is different for any of your 4 groups. The p-values show that no, none of the slopes are statistically different from each other or from zero.
# 
# The only "hint" of a relationship is the term log10...:diet_treatmentsucrose_water (p=0.0765), which suggests that adding sucrose might change the health impact in biapenem-treated mice, but the effect isn't strong enough to be statistically significant.
# 
# Devil's Advocate 😈
# With 8 terms in your model and a limited number of mice, your model is "over-parameterized." You are asking a lot of questions of a (likely) small dataset. This splits your statistical power, making it very hard for any term to become significant. The model is "confused" by all the comparisons. Sometimes, a simpler model (like the one you ran first) is more powerful and easier to interpret.
```

## final version

```{r}
# only looking at the regular water group
faceted_plot_regularwater <- bloom_data_percent %>%
  # Filter to only include Day 3 data
  filter(day == 3) %>%
  # Initialize the ggplot object
  ggplot(aes(
  ...
  # Clean up the labels to be more readable
  labs(
    subtitle = 'Mice on regular water diet ',
    x = "Enterococcus Bloom (Log10 CFU/gram stool)",
    y = "Weight Change on Day 3 from Baseline (%)",
    color = "Antibiotic"
  ) +
  # Use a clean theme
  theme_bw() +
...
# To display the plot, just run the variable name
print(faceted_plot_regularwater)
```
```{r}
day3_data_regularwater <- bloom_data_percent %>%
  # We only want to plot the measurements from Day 3 (the bloom day)
  filter(day == 3) |> 
  mutate(antibiotic_treatment = factor(antibiotic_treatment, levels = c('biapenem','PBS')))

interaction_model_regularwater <- lm(
  percent_change ~ log10_cf_us_per_gram_stool * antibiotic_treatment ,
  data = day3_data_regularwater
)

# --- 4. View the Model Results ---
# Using tidy() from the 'broom' package gives a clean table
# instead of the standard R summary() output.
model_summary_regular <- tidy(interaction_model_regularwater)

print(model_summary_regular)
#This new output actually gives you a very important piece of information. Look at the log10_cf_us_per_gram_stool term (row 2).
# 
# Term: log10_cf_us_per_gram_stool
# 
# Estimate: -0.549
# 
# p.value: 0.0986
# 
# In this model, this term represents the slope for the biapenem group. That p-value of 0.0986 is greater than 0.05, which means that even the negative slope for the biapenem group by itself is not statistically significant.
# 
# Final Conclusion: You have now confirmed this from two different angles. The statistical conclusion is solid: this dataset does not provide evidence for a weight loss associated with the Enterococcus bloom, as neither group's slope is significantly different from zero, and the slopes are not significantly different from each other.
```
```{r}
data_with_groups <- day3_data_regularwater %>%
  mutate(
    bloom_group = if_else(
      log10_cf_us_per_gram_stool < 5.0, 
      "Low Bloom (<5)", 
      "High Bloom (>=5)"
    )
  )

# --- 3. New Visualization (Boxplot) ---
# This plot directly tests the reviewer's idea.
# We plot the four new groups.
new_plot <- data_with_groups %>%
  ggplot(aes(
    x = antibiotic_treatment, 
    y = percent_change, 
    fill = bloom_group
  )) +
  geom_boxplot(
    # Use 'outlier.shape = NA' to hide boxplot outliers
    # We will show them with geom_jitter() instead
    outlier.shape = NA 
  ) +
  geom_jitter(
    # 'position_jitterdodge' makes the dots dodge so they
    # align with their correct boxplot.
    alpha = 0.7,
    position = position_jitterdodge(jitter.width = 0.2)
  ) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
  labs(
    title = "Health Impact by Bloom Group (Day 3)",
    subtitle = "Comparing Low vs. High Bloom Clusters",
    x = "Antibiotic Treatment",
    y = "Weight Change from Baseline (%)",
    fill = "Bloom Group" # This sets the legend title
  ) +
  theme_bw()

print(new_plot)
# the data doesn't show a correlation, it shows a causal split. The antibiotic creates a high-bloom "disease state" that is linked to weight loss. Mice that resist this (the biapenem "Low Bloom" group) are perfectly healthy, just like the PBS controls.
```
### the sucrose water treatment          

```{r}
# Filter for Day 3 and 'sucrose_water'
day3_sucrose_data <- plot_data_percent %>%
  filter(
    day == 3,
    diet_treatment == "sucrose_water"
  ) |> 
  mutate(antibiotic_treatment = factor(antibiotic_treatment, levels = c('biapenem','PBS')))

# --- 2. Create the New Bloom "Groups" ---
# Create the same categorical variable based on the 5.0 logCFU cutoff.
data_with_groups_sucrose <- day3_sucrose_data %>%
  mutate(
    bloom_group = if_else(
      log10_cf_us_per_gram_stool < 5.0, 
      "Low Bloom (<5)", 
...
# --- 3. New Visualization (Boxplot) ---
# This plot shows the sucrose_water data only.
sucrose_plot <- data_with_groups_sucrose %>%
  ggplot(aes(
    x = antibiotic_treatment, 
    y = percent_change, 
    fill = bloom_group
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
  labs(
    title = "Health Impact by Bloom Group (Sucrose Water, Day 3)",
    subtitle = "Comparing Low vs. High Bloom Clusters",
    x = "Antibiotic Treatment",
    y = "Weight Change from Baseline (%)",
    fill = "Bloom Group"
...
```{r}
# --- 4. New Statistical Model (Two-Way ANOVA) ---
# This group also has an "empty cell" (no 'High Bloom - PBS' mice),
# so we fit the same "additive model" as before.
# This tests the main effects of bloom group and antibiotic treatment.

anova_model_sucrose <- aov(
  percent_change ~ bloom_group + antibiotic_treatment, 
  data = data_with_groups_sucrose
)

# Use tidy() to see the results in a clean table
tidy(anova_model_sucrose)
...
```{r}
# plot the above two together on the same scale 
day3_data <- plot_data_percent %>%
  filter(
    day == 3
  ) |> 
...
# --- 2. Create the New Bloom "Groups" ---
# Create the categorical variable based on the 5.0 logCFU cutoff
# for all the Day 3 data.
data_with_groups <- day3_data %>%
  mutate(
    bloom_group = if_else(
      log10_cf_us_per_gram_stool < 5.0, 
      "Low Bloom (<5)", 
...
# --- 3. New Visualization (Faceted Boxplot) ---
# This plot shows both diet treatments side-by-side
faceted_plot <- data_with_groups %>%
  ggplot(aes(
    x = antibiotic_treatment, 
    y = percent_change, 
    fill = bloom_group
  ...
  # --- This is the new line that creates the facets ---
  facet_wrap(~ diet_treatment) +
  # ---
  
  labs(
    title = "Health Impact by Bloom Group and Diet (Day 3)",
    subtitle = "Comparing Low vs. High Bloom Clusters Across Diets",
    x = "Antibiotic Treatment",
    y = "Weight Change from Baseline (%)",
    fill = "Bloom Group"
  ) +
...
# Analyze each diet group separately
# to compare the impact of the bloom group in each condition.

# --- Model for regular_water ---
regular_data_grouped <- data_with_groups %>%
  filter(diet_treatment == "regular_water")

anova_model_regular <- aov(
  percent_change ~ bloom_group + antibiotic_treatment, 
  data = regular_data_grouped
)
print("--- Regular Water ANOVA ---")
print(tidy(anova_model_regular))


# --- Model for sucrose_water ---
sucrose_data_grouped <- data_with_groups %>%
  filter(diet_treatment == "sucrose_water")

anova_model_sucrose <- aov(
  percent_change ~ bloom_group + antibiotic_treatment, 
  data = sucrose_data_grouped
)
print("--- Sucrose Water ANOVA ---")
print(tidy(anova_model_sucrose))
...
```{r}
# --- 4. New Statistical Model (Two-Way ANOVA) ---
# Because there is an "empty cell" in the data (no 'High Bloom - PBS' mice),
# we cannot test the interaction.
# Therefore, we fit an "additive model" using '+' instead of '*'.
# This model tests the two "main effects" that the data can support:
# 1. Main effect of bloom_group
# 2. Main effect of antibiotic_treatment
...
# Use tidy() to see the results in a clean table
# This is the statistical output for your boxplot.
tidy(anova_model)
...
# % change in abx treated mice on day 3 only 

```{r}
biapenem_day3_data <- plot_data_percent %>%
  filter(
    day == 3,
    # Use str_detect to catch "biapenem" or "Biapenem"
    stringr::str_detect(antibiotic_treatment, "biapenem")
  )%>%
  mutate(
    diet_treatment = paste("biapenem +", diet_treatment)
  )

# --- 2. Run Linear Models to Get p-values AND Estimates ---
# Run a separate model for each diet group to get stats for each slope

# Model for regular_water
lm_regular_stats <- biapenem_day3_data %>% 
  filter(diet_treatment == "biapenem + regular_water") %>%
  lm(percent_change ~ log10_cf_us_per_gram_stool, data = .) %>%
  broom::tidy() %>%
  filter(term == "log10_cf_us_per_gram_stool")

p_regular <- lm_regular_stats$p.value
est_regular <- lm_regular_stats$estimate

# Model for sucrose_water
lm_sucrose_stats <- biapenem_day3_data %>%
  filter(diet_treatment == "biapenem + sucrose_water") %>%
  lm(percent_change ~ log10_cf_us_per_gram_stool, data = .) %>%
  broom::tidy() %>%
  filter(term == "log10_cf_us_per_gram_stool")
  
p_sucrose <- lm_sucrose_stats$p.value
est_sucrose <- lm_sucrose_stats$estimate

# --- 3. Create Annotation Data Frame ---
# This data frame will hold the labels and their coordinates
annotations <- tibble(
  # This column must match the new facet labels
  diet_treatment = c("biapenem + regular_water", "biapenem + sucrose_water"), 
  
  # --- NEW: Create labels with both Estimate and p-value ---
  # Using sprintf() for clean formatting (%.2f = 2 decimal places)
  slope_label = c(
    sprintf("Slope: %.2f\np = %.3f", est_regular, p_regular),
    sprintf("Slope: %.2f\np = %.3f", est_sucrose, p_sucrose)
  ),
  
  # --- NEW: Coordinates for top-left corner ---
  # We use min(x) and max(y) to place it relative to the data
  x_coord = c(0, 2.5), # approx min x for each panel
  y_coord = c(6, 6)  # approx max y for each panel
)

# --- 4. Generate the Faceted Scatter Plot ---
# This plot will only have biapenem mice, split by diet
biapenem_plot <- biapenem_day3_data %>%
  ggplot(aes(
    x = log10_cf_us_per_gram_stool,
    y = percent_change
  )) +
  geom_point(alpha = 0.8, size = 3) +
  # Add regression lines for each group (will be colored by diet)
  geom_smooth(method = "lm", se = TRUE) +
  # Add the 0% line for reference
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
  
  # --- Add the new p-value and estimate labels ---
  geom_text(
    data = annotations, 
    aes(x = x_coord, y = y_coord, label = slope_label),
    hjust = "left",   # --- NEW: Justify text to the left
    vjust = "top",    # --- NEW: Justify text to the top
    size = 3.5,
    fontface = "italic"
  ) +
  
  # --- Facet by diet_treatment (will use the new labels) ---
  facet_wrap(~ diet_treatment) +
  
  labs(
    title = "Weight change in Biapenem-Treated Mice (Day 3)",
    subtitle = "Comparing Regular Water vs. Sucrose Water Diets",
    x = "Enterococcus (Log10 CFU/gram stool)",
    y = "Weight Change from Baseline (%)",
    color = "Diet Treatment"
  ) +
  theme_bw(base_size = 12) +
...
# Display the plot 
print(biapenem_plot)  
# ggsave() does not save the ggplot object for later editing.
saveRDS(biapenem_plot, file = "../data/R31_biapenem_plot_weight_perc_change.RDS")

# how many independent experiments and how many mice 
biapenem_day3_data |> count(experiment_no) |> nrow()

biapenem_day3_data |> count(experiment_no, diet_treatment) 
```

### weight % overlay

```{r}
# make a plot where the X is the day, and the y is the percent change of wegiht from baseline, and the is a line conecting the median, and the error bar of the y in each tretment group (antibiotic treatment + diet treatment ) are shown , use the color palette "PBS + Water" = "lightgray", "PBS + Sucrose" = "lightpink","Abx + Water" = "gray32", "Abx + Sucrose "="deeppink2",
df_plot_ready <- plot_data_percent %>%
  mutate(treatment_group = case_when(
    grepl("PBS_regular_water", plot_group) ~ "PBS + Water",
    grepl("PBS_sucrose_water", plot_group) ~ "PBS + Sucrose",
    grepl("biapenem_regular_water", plot_group) ~ "Abx + Water",
    grepl("biapenem_sucrose_water", plot_group) ~ "Abx + Sucrose"
  ))

# Now, summarize the data to get the median and interquartile range (for error bars)
# for each treatment group at each day.
summary_data <- df_plot_ready %>%
  group_by(treatment_group, day) %>%
  summarise(
    median_change = median(percent_change, na.rm = TRUE),
    # For error bars, we'll use the 25th (q1) and 75th (q3) percentiles
    q1 = quantile(percent_change, 0.25, na.rm = TRUE),
    q3 = quantile(percent_change, 0.75, na.rm = TRUE),
    .groups = 'drop' # Good practice to drop the grouping
  )
...
color_palette <- c(
  ...
  "PBS + Sucrose" = "lightpink",
  "Abx + Water" = "gray32",
  "Abx + Sucrose" = "deeppink2"
)

# 1. Define the amount of dodging. 
# This keeps the value consistent for all layers.
# You can experiment with changing the width (e.g., to 0.4).
dodge_width <- position_dodge(width = 0.1)

# 2. Apply `position = dodge_width` to all geoms that act on the x-axis.
weight_change4 <- ggplot(summary_data, aes(x = day, y = median_change, group = treatment_group, color = treatment_group)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  labs(
    title = "Median Percent Weight Change from Baseline by Treatment",
    x = "Day of Experiment",
    y = "Weight Change from Baseline (%)",
...
weight_change4
save_ggplot_data_to_excel(weight_change4, summary_data, "../mouse_data.xlsx", "Sup_Figure_8m_mouse_percent_weight_change", col_names = c(
  "day" = "Day",
  "median_change" = "Median Weight Change",
  "treatment_group" = "Treatment Group",
  "q1" = "Quantile 1",
  "q3" = "Quantile 3"))
save_full_dataframe_to_excel(df_plot_ready %>% select(c(percent_change, day, treatment, mice_weights, mouse_identifier, experiment_no, baseline_weight)), "../mouse_data.xlsx", "Sup_Figure_8m_all_mouse_data", col_names = c(
  "day" = "Day",
  "mice_weights" = "Mouse Weight",
  "treatment" = "Treatment Group",
  "mouse_identifier" = "Mouse Identifier",
  "experiment_no" = "Experiment Number",
  "baseline_weight" = "Baseline Weight",
  "percent_change" = "Weight Percent Change from Baseline"))
saveRDS(weight_change4, file = "../data/R31_weight_change4_perc.RDS")
```
### vardi test 

```{r}
# Why Informative Censoring Matters
# In plain terms, "informative censoring" is when a subject (like a mouse) is removed from the experiment for a reason related to what you're measuring.
# 
# The Paper's Example: An animal is sacrificed "for humane reasons if its tumour becomes too large".
# 
# Your Experiment's Example: You might have to remove a mouse from the study because it lost too much weight or became too sick (which is what your weight-change graph is measuring!).
# 
# This creates a serious bias. A simple test (like the Kruskal-Wallis I suggested) would be skewed. Why? Because the mice with the strongest effect (e.g., biggest tumor, most weight loss) are removed early, making their AUC look artificially small. This test is built to correct that bias.
#How It Works (The Simple Version)
# Instead of just calculating one AUC for each mouse and comparing the group averages, this Vardi test does something more clever:
# 
# It compares every possible pair of mice, with one mouse from Group A and one from Group B.
# 
# For each pair, it calculates the difference in their curves (e.g., Mouse A weight - Mouse B weight).
# 
# Critically, it only calculates this difference up to the time point that both mice were still in the study.
# 
# It then averages all these pairwise comparisons to get a final test statistic.
library(clinfun)
library(utils) # For combn

wide_data <- df_plot_ready %>%
  # Keep only the data we need
  dplyr::select(mouse_identifier, treatment_group, day, percent_change) %>%
  
  # Pivot to the "wide" format
  pivot_wider(
    id_cols = c(mouse_identifier, treatment_group),
    names_from = day,
    values_from = percent_change,
    names_prefix = "Day_" # Add prefix to keep column names clean
  )
```
...
```{r}
# ---
# Step 2: Prepare inputs for the function
# ---

# The function needs the measurement matrix (meas) and the group vector (grp)
# We must use as.matrix() for the measurements
meas_matrix <- wide_data %>%
  dplyr::select(starts_with("Day_")) %>%
  as.matrix()

# The group vector
group_vector <- wide_data$treatment_group

# The time points (as a numeric vector)
# We extract the numbers from the column names
time_points <- as.numeric(gsub("Day_", "", colnames(meas_matrix)))

# ---
# Step 3: Run pairwise tests
# ---

# Get all unique group names
all_groups <- unique(group_vector)

# Create all possible 2-group combinations
all_pairs <- combn(all_groups, 2, simplify = FALSE)

# Run the test for each pair
test_results <- lapply(all_pairs, function(pair) {
  
  # Run the Vardi test, specifying which two groups to compare
  test <- aucVardiTest(
    meas = meas_matrix,
    grp = group_vector,
    tim = time_points,
    cgrps = pair,
    nperm = 5000 # Number of permutations for the p-value
  )
  
  # Return a clean data frame with the results
  data.frame(
    Group1 = pair[1],
    Group2 = pair[2],
...
print("--- Vardi Test Pairwise Comparisons (p-values adjusted with 'BH') ---")
print(results_df)
```
