...
# how many mouse
#weight_data3 %>% count(experiment_no, mouse_identifier) %>% count(experiment_no)
```
...
### the weight trajectories of mice 
...
ggplot(weight_data3, aes(x = day, y = mice_weights)) +
 ...
 stat_summary(
  ...
  labs(
    title = "Antibiotics treated mice",
    x = "Day",
    y = "Mouse Weight (g)"
  ) +
  theme_bw()
...
ggplot(predicted_effects, aes(x = x, y = predicted, color = group)) +
  ...
  geom_point(
    data = weight_data2,
    aes(x = log10_cf_us_per_gram_stool, y = mice_weights, color = diet_treatment,shape =  day),
    alpha = 0.5,
