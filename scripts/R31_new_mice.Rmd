...
weights <- bind_rows(
  w1 %>%  select(experiment_no, group = room, day, mouse_identifier, mice_weights = weight, treatment),
...
### trajectories of mice 

```{r}
# weight_data3 only has all mice that had all three data points. 
...
timeline <- ggplot(weight_data3, aes(x = day, y = mice_weights)) +
...
predicted_effects <- ggpredict(weight_model3, terms = c("log10_cf_us_per_gram_stool", "diet_treatment"))
...
# 2. Create the plot
ggplot(plot_data_percent, aes(x = day, y = percent_change)) +
...
```{r}
# a subset of the above that I exclude the survey_RB08 experiment since I don't have the CFU
plot_data_percent_subset <- plot_data_percent %>% 
  filter(experiment_no != 'survey_RB08')
  
# 2. Create the plot
ggplot(plot_data_percent_subset, aes(x = day, y = percent_change)) +
 ...
 # Calculate and plot a line connecting the medians.
  ...
  # Add informative labels and a clean theme
  labs(
    title = "Mouse Weight Percent Change from Baseline by Diet\nexcluding the survey_RB08 experiment",
    x = "Day",
    y = "Weight Change from Baseline (%)"
  ) +
  theme_bw()
...
percent_change_model <- lmer(
  ...
  data = plot_data_percent_subset %>% filter(day != '1')
)
...
weight_data3 %>% filter(day  == '1') %>% count(experiment_no, diet_treatment) %>% 
  spread('diet_treatment','n') %>% 
  mutate(experiment_no = seq(1, nrow(.)),
        experiment_no= str_glue('Experiment_{experiment_no}') ) %>% 
  gt()  %>% 
...
```{r}
tally <- ggdraw() + draw_image("../data/R31_mice_tally.png", scale =5.5)

weight_plot <-  plot_grid(tally,
                          timeline,
                          scatter,
                 label_size = 12, nrow  = 3,
                 rel_heights = c(.5,.8,1),labels = "auto", 
                align = 'vh', axis = 'lrtb')
...
combined <- plot_grid(
  ...
  # rel_heights values control vertical title margins
  rel_heights = c(0.07, 1)
) +theme(plot.margin = unit(c(1,10,1,2), "cm"))
