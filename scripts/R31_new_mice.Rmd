...
# Data definition for points and segments
point_data <- tibble::tribble(
  ~group,                ~event,             ~day,
  "Standard",            "Antibiotic",       1, "Delay +1",            "Antibiotic",       1, "Delay +2",            "Antibiotic",       1,
  "Standard",            "Stool Collection", 1, "Delay +1",            "Stool Collection", 1, "Delay +2",            "Stool Collection", 1,
  "Standard",            "Stool Collection", 3, "Delay +1",            "Stool Collection", 3, "Delay +2",            "Stool Collection", 3,
  "Standard",            "Stool Collection", 6, "Delay +1",            "Stool Collection", 6, "Delay +2",            "Stool Collection", 6,
  "Standard",            "Stool Collection", 9, "Delay +1",            "Stool Collection", 9, "Delay +2",            "Stool Collection", 9
)

segment_data <- tibble::tribble(
    ~group,      ~start, ~end,
    "Standard",  1,      6,
    "Delay +1",  2,      6,
    "Delay +2",  3,      6
...
group_order <- c("Standard", "Delay +1", "Delay +2")
point_data$group <- factor(point_data$group, levels = group_order)
segment_data$group <- factor(segment_data$group, levels = group_order)

# --- Fully Corrected ggplot Code ---
ggplot() +
  ...
  geom_point(
    data = point_data,
    aes(x = day, y = 0, shape = event, color = event),
    size = 6,
    fill = "white",
    stroke = 1.2
  ) +
  geom_point(aes(x = -100, y = -100, fill = "Sucrose"), shape = 22, size = 5) +
  scale_fill_manual(
    name = NULL,
    values = c("Sucrose" = "#E41A1C"),
    labels = c("Sucrose" = "Sucrose Treatment")
  ) +
  facet_wrap(~group, ncol = 1, strip.position = "left") +
  scale_color_manual(
    name = "Event",
    values = c("Antibiotic" = "#377EB8", "Stool Collection" = "black"),
    labels = c("Antibiotic" = "Single Antibiotic Dose", "Stool Collection" = "Stool Collection")
  ) +
  scale_shape_manual(
    name = "Event",
    values = c("Antibiotic" = 24, "Stool Collection" = 16),
    labels = c("Antibiotic" = "Single Antibiotic Dose", "Stool Collection" = "Stool Collection")
  ) +
  labs(
    title = "Experimental Design: Delayed Sucrose Administration",
    x = "Day of Experiment",
    y = NULL
  ) +
  ...
  ) + # The '+' now correctly connects theme() to the next component
