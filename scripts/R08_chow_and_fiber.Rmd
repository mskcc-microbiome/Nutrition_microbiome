...
chow_days <- chow %>% 
  ggboxplot(x = 'treatment', y = 'food_consumed_g', facet.by =  'day', color = 'treatment',
            xlab = '', ylab = 'Chow consumption (grams)\nper mouse per day',
            add = 'jitter', 
            outlier.shape = NA,
            palette = c('gray32','deeppink2')) 
...
chow_AUC <- chow %>% 
  select(-total_food_g, -food_left_g) %>% 
  pivot_wider(id_cols = c(diet_treatment,
                          abx_treatment,
                          mouse),
              names_from = day,
              names_prefix = "",
              values_from = food_consumed_g)
...
chow_AUC_wide <- chow_AUC %>% 
  mutate(trap_3_6 = (Day3  + Day6 )*3/2,
         ...
         groups = paste(abx_treatment,
                         diet_treatment,sep="+")) %>% 
  mutate(groups = factor(groups, levels = c('biapenem+vehicle','biapenem+sucrose')))
```


```{r}
chow_AUC_plot <- ggboxplot(chow_AUC_wide,
          x="groups",add = 'jitter', title = 'Chow consumption AUC day 3,6,9',
          y="trap", color  = 'groups', add.params = list(alpha = 0.8, shape = 16)) +
  ylab("Trapezoidal\nAUC")+xlab('') +
   scale_color_manual(values = c('gray32','deeppink2')) +
  stat_compare_means(comparisons = list( 
                        c("biapenem+vehicle", "biapenem+sucrose")),label= "p.signif", method= 'wilcox.test',tip.length = 0.04,exact=T,correct=T) +
