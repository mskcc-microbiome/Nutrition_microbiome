...
cluster_cal <- p_cal %>% 
  ggplot(aes(x = fdrt, y = d_Calories_kcal, fill = modal_diet, color = modal_diet)) +
        ...
        labs(title = 'calories',y = 'Daily intake',x = 'HCT Day') +
  ...
  scale_fill_manual(values = c("darkgoldenrod2", "darkslateblue")) +
        facet_wrap(~ modal_diet ) +theme_bw()  +  theme(aspect.ratio = 3.5, legend.position = 'none', strip.text = element_blank())
...
combined <- plot_grid(plotlist = c(prop_plots, cluster_cal), nrow = 1)
combined
```
```{r}
res <- plot_grid(
...
ggsave('../data/E06cde_R10_clinical_proportions.pdf',
