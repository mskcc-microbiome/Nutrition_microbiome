...
```
```{r}
#  x% of the points after day 20 are orange, and y% of the points after day 30 are orange.
daily_intake_final |> 
  filter(fdrt > 20) |> 
  count(intake_source) |> 
...
sweets_plot <- ggplot(daily_intake_final, aes(x = fdrt, y = total_sweets_dehydrated_weight, color = had_enteral)) +
  ...
  geom_smooth(aes(x = fdrt, y = total_sweets_dehydrated_weight), method = "loess", formula = 'y ~ x', colour = diet_line_color ,  size = 1, fill = 'hotpink') +
...
sugars_plot <- ggplot(daily_intake_final, aes(x = fdrt, y = total_sugars_g, color = intake_source)) +
  geom_point(alpha = scatter_transparency, size = 1.5) +
  geom_smooth(aes(x = fdrt, y = total_sugars_g), method = "loess", formula = 'y ~ x', colour = diet_line_color ,  size = 1, fill = 'hotpink') +
...
ggsave('../data/E04h_R47_sugars_and_sweets_discrepancy.pdf',
